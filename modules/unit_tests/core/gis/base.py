...
# python web2py.py -S eden -M -R applications/eden/modules/unit_tests/core/gis/base.py
...
# =============================================================================
class LocationTreeTests(unittest.TestCase):
    ...
    def testULT1_update_location_tree_disabled(self):
        ...
        assertEqual(L1_record.inherited, False)
        assertEqual(L1_record.path, None)
        assertEqual(L1_record.lat, None)
        assertEqual(L1_record.lon, None)
        ...
        assertEqual(L1_record.inherited, True)
        assertEqual(L1_record.path, "%s/%s" %(L0_id, L1_id))
        assertEqual(L1_record.lat, L0_lat)
        assertEqual(L1_record.lon, L0_lon)
    ...
    def testULT2_update_location_tree_all_locations(self):
        ...
        assertEqual(L2_record.inherited, True)
        assertEqual(L2_record.path, "%s/%s/%s" % (L0_id, L1_id, L2_id))
        assertEqual(L2_record.lat, L0_lat)
        assertEqual(L2_record.lon, L0_lon)
        specific_record = db(table.id == specific_id).select(*self.fields,
                                                             limitby=(0, 1)
                                                             ).first()
        assertEqual(specific_record.inherited, True)
        assertEqual(specific_record.path, "%s/%s/%s/%s" % (L0_id, L1_id, L2_id, specific_id))
        assertEqual(specific_record.lat, L0_lat)
        assertEqual(specific_record.lon, L0_lon)
    ...
    def _testL0(self, with_level):
        ...
        LocationTreeTests.ids[L0] = L0_id
        ...
        assertEqual = self.assertEqual
        assertEqual(record.inherited, False)
        self.assertAlmostEqual(record.lat, 26.969696969696972, 13)
        self.assertAlmostEqual(record.lon, 25.454545454545453, 13)
        assertEqual(record.lat_min, 10)
        assertEqual(record.lat_max, 40)
        assertEqual(record.lon_min, 10)
        assertEqual(record.lon_max, 40)
        assertEqual(record.parent, None)
        assertEqual(record.path, "%s" % L0_id)
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, None)
        assertEqual(record.L2, None)
        assertEqual(record.L3, None)
        assertEqual(record.L4, None)
        assertEqual(record.L5, None)
        if self.spatialdb:
    ...
    def _testL1(self, with_level):
        ...
        # Store for future tests
        LocationTreeTests.ids[L1] = L1_id
        ...
        assertEqual(record.lat_min, 11)
        assertEqual(record.lat_max, 39)
        assertEqual(record.lon_min, 11)
        assertEqual(record.lon_max, 39)
        assertEqual(record.parent, L0_id)
        assertEqual(record.path, "%s/%s" % (L0_id, L1_id))
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, L1)
        assertEqual(record.L2, None)
        assertEqual(record.L3, None)
        assertEqual(record.L4, None)
        assertEqual(record.L5, None)
    ...
    def _testL2(self, with_level):
        ...
        # Store for future tests
        LocationTreeTests.ids[L2] = L2_id
        ...
        assertEqual(record.lat_min, 12)
        assertEqual(record.lat_max, 38)
        assertEqual(record.lon_min, 12)
        assertEqual(record.lon_max, 38)
        assertEqual(record.parent, L1_id)
        assertEqual(record.path, "%s/%s/%s" % (L0_id, L1_id, L2_id))
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, L1)
        assertEqual(record.L2, L2)
        assertEqual(record.L3, None)
        assertEqual(record.L4, None)
        assertEqual(record.L5, None)
        if self.spatialdb:
    ...
    def _testL3(self, with_level):
        ...
        # Store for future tests
        LocationTreeTests.ids[L3] = L3_id
        ...
        assertEqual(record.lat_min, 13)
        assertEqual(record.lat_max, 37)
        assertEqual(record.lon_min, 13)
        assertEqual(record.lon_max, 37)
        assertEqual(record.parent, L2_id)
        assertEqual(record.path, "%s/%s/%s/%s" % (L0_id, L1_id, L2_id, L3_id))
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, L1)
        assertEqual(record.L2, L2)
        assertEqual(record.L3, L3)
        assertEqual(record.L4, None)
        assertEqual(record.L5, None)
        if self.spatialdb:
            self.assertTrue(record.the_geom is not None)
    ...
    def _testL4(self, with_level):
        ...
        # Store for future tests
        LocationTreeTests.ids[L4] = L4_id
        ...
        assertEqual(record.lat_min, 14)
        assertEqual(record.lat_max, 36)
        assertEqual(record.lon_min, 14)
        assertEqual(record.lon_max, 36)
        assertEqual(record.parent, L3_id)
        assertEqual(record.path, "%s/%s/%s/%s/%s" % (L0_id, L1_id, L2_id, L3_id, L4_id))
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, L1)
        assertEqual(record.L2, L2)
        assertEqual(record.L3, L3)
        assertEqual(record.L4, L4)
        assertEqual(record.L5, None)
        if self.spatialdb:
            self.assertTrue(record.the_geom is not None)
    ...
    def _testL5(self, with_level):
        ...
        assertEqual(record.lat_min, 15)
        assertEqual(record.lat_max, 35)
        assertEqual(record.lon_min, 15)
        assertEqual(record.lon_max, 35)
        assertEqual(record.parent, L4_id)
        assertEqual(record.path, "%s/%s/%s/%s/%s/%s" % (L0_id, L1_id, L2_id, L3_id, L4_id, L5_id))
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, L1)
        assertEqual(record.L2, L2)
    ...
    def _testL3_L1_parent(self, with_level):
        ...
        assertEqual(record.lon_max, 37)
        assertEqual(record.parent, L1_id)
        assertEqual(record.path, "%s/%s/%s" % (L0_id, L1_id, L3_id))
        assertEqual(record.wkt, POLYGON)
        assertEqual(record.L0, L0)
        assertEqual(record.L1, L1)
        assertEqual(record.L2, None)
...
# =============================================================================
class NoGisConfigTests(unittest.TestCase):
    """
