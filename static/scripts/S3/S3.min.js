...
p[0].nodeType;return{element:p,isWindow:r,isDocument:v,offset:r||v?{left:0,top:0}:a(m).offset(),scrollLeft:p.scrollLeft(),scrollTop:p.scrollTop(),width:p.outerWidth(),height:p.outerHeight()}}};a.fn.position=function(m){if(!m||!m.of)return n.apply(this,arguments);m=a.extend({},m);var p="string"===typeof m.of?a(document).find(m.of):a(m.of),r=a.position.getWithinInfo(m.within),v=a.position.getScrollInfo(r),x=(m.collision||"flip").split(" "),B={};var t=d(p);p[0].preventDefault&&(m.at="left top");var u=
t.width;var y=t.height;var w=t.offset;var C=a.extend({},w);a.each(["my","at"],function(){var z=(m[this]||"").split(" ");1===z.length&&(z=h.test(z[0])?z.concat(["center"]):f.test(z[0])?["center"].concat(z):["center","center"]);z[0]=h.test(z[0])?z[0]:"center";z[1]=f.test(z[1])?z[1]:"center";var D=k.exec(z[0]);var F=k.exec(z[1]);B[this]=[D?D[0]:0,F?F[0]:0];m[this]=[q.exec(z[0])[0],q.exec(z[1])[0]]});1===x.length&&(x[1]=x[0]);"right"===m.at[0]?C.left+=u:"center"===m.at[0]&&(C.left+=u/2);"bottom"===m.at[1]?
C.top+=y:"center"===m.at[1]&&(C.top+=y/2);var A=b(B.at,u,y);C.left+=A[0];C.top+=A[1];return this.each(function(){var z,D=a(this),F=D.outerWidth(),E=D.outerHeight(),G=parseInt(a.css(this,"marginLeft"),10)||0,I=parseInt(a.css(this,"marginTop"),10)||0,H=F+G+(parseInt(a.css(this,"marginRight"),10)||0)+v.width,J=E+I+(parseInt(a.css(this,"marginBottom"),10)||0)+v.height,M=a.extend({},C),R=b(B.my,D.outerWidth(),D.outerHeight());"right"===m.my[0]?M.left-=F:"center"===m.my[0]&&(M.left-=F/2);"bottom"===m.my[1]?
M.top-=E:"center"===m.my[1]&&(M.top-=E/2);M.left+=R[0];M.top+=R[1];var S={marginLeft:G,marginTop:I};a.each(["left","top"],function(L,O){if(a.ui.position[x[L]])a.ui.position[x[L]][O](M,{targetWidth:u,targetHeight:y,elemWidth:F,elemHeight:E,collisionPosition:S,collisionWidth:H,collisionHeight:J,offset:[A[0]+R[0],A[1]+R[1]],my:m.my,at:m.at,within:r,elem:D})});m.using&&(z=function(L){var O=w.left-M.left,P=O+u-F,Q=w.top-M.top,U=Q+y-E,N={target:{element:p,left:w.left,top:w.top,width:u,height:y},element:{element:D,
...
m.top)}},flip:{left:function(m,p){var r=p.within,v=r.offset.left+r.scrollLeft,x=r.width,B=r.isWindow?r.scrollLeft:r.offset.left,t=m.left-p.collisionPosition.marginLeft;r=t-B;var u=t+p.collisionWidth-x-B;t="left"===p.my[0]?-p.elemWidth:"right"===p.my[0]?p.elemWidth:0;var y="left"===p.at[0]?p.targetWidth:"right"===p.at[0]?-p.targetWidth:0,w=-2*p.offset[0];if(0>r){if(p=m.left+t+y+w+p.collisionWidth-x-v,0>p||p<e(r))m.left+=t+y+w}else 0<u&&(p=m.left-p.collisionPosition.marginLeft+t+y+w-B,0<p||e(p)<u)&&
(m.left+=t+y+w)},top:function(m,p){var r=p.within,v=r.offset.top+r.scrollTop,x=r.height,B=r.isWindow?r.scrollTop:r.offset.top,t=m.top-p.collisionPosition.marginTop;r=t-B;var u=t+p.collisionHeight-x-B;t="top"===p.my[1]?-p.elemHeight:"bottom"===p.my[1]?p.elemHeight:0;var y="top"===p.at[1]?p.targetHeight:"bottom"===p.at[1]?-p.targetHeight:0,w=-2*p.offset[1];if(0>r){if(p=m.top+t+y+w+p.collisionHeight-x-v,0>p||p<e(r))m.top+=t+y+w}else 0<u&&(p=m.top-p.collisionPosition.marginTop+t+y+w-B,0<p||e(p)<u)&&(m.top+=
...
"isRTL");var r=this._get(f,"showButtonPanel");var v=this._get(f,"hideIfNoPrevNext");var x=this._get(f,"navigationAsDateFormat");var B=this._getNumberOfMonths(f),t=this._get(f,"showCurrentAtPos");var u=this._get(f,"stepMonths");var y=1!==B[0]||1!==B[1],w=this._daylightSavingAdjust(f.currentDay?new Date(f.currentYear,f.currentMonth,f.currentDay):new Date(9999,9,9)),C=this._getMinMaxDate(f,"min"),A=this._getMinMaxDate(f,"max");t=f.drawMonth-t;var z=f.drawYear;0>t&&(t+=12,z--);if(A){var D=this._daylightSavingAdjust(new Date(A.getFullYear(),
...
var n=1==this._get(c,"showMinutes"),m=this._get(c,"amPmText"),p=this._get(c,"rows"),r=0,v=0,x=h=0,B=0,t=0,u=[],y=this._get(c,"hours"),w=0;var C=this._get(c,"hourText");var A=this._get(c,"showCloseButton"),z=this._get(c,"closeButtonText"),D=this._get(c,"showNowButton"),F=this._get(c,"nowButtonText"),E=this._get(c,"showDeselectButton"),G=this._get(c,"deselectButtonText"),I=A||D||E;for(e=y.starts;e<=y.ends;e++)u.push(e);e=Math.ceil(u.length/p);if(k){for(w=0;w<u.length;w++)12>u[w]?h++:x++;w=0;r=Math.floor(h/
u.length*p);v=Math.floor(x/u.length*p);p!=r+v&&(h&&(!x||!r||v&&h/r>=x/v)?r++:v++);B=Math.min(r,1);t=r+1;e=0==r?Math.ceil(x/v):0==v?Math.ceil(h/r):Math.ceil(Math.max(h/r,x/v))}h='<table class="ui-timepicker-table ui-widget-content ui-corner-all"><tr>';if(l){h+='<td class="ui-timepicker-hours"><div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+C+'</div><table class="ui-timepicker">';for(l=1;l<=p;l++){h+="<tr>";l==B&&k&&(h+='<th rowspan="'+r.toString()+'" class="periods" scope="row">'+
...
n[p+"Text"]+'</dt><dd class="ui_tpicker_'+p+(v?"":" ui_tpicker_unit_hide")+'"><div class="ui_tpicker_'+p+"_slider"+(v?"":" ui_tpicker_unit_hide")+'"></div>';if(v&&0<n[p+"Grid"]){t+='<div style="padding-left: 1px"><table class="ui-tpicker-grid-label"><tr>';if("hour"===p)for(r=n[p+"Min"];r<=x[p];r+=parseInt(n[p+"Grid"],10))B[p]++,v=a.datepicker.formatTime(this.support.ampm?"hht":"HH",{hour:r},n),t+='<td data-for="'+p+'">'+v+"</td>";else for(r=n[p+"Min"];r<=x[p];r+=parseInt(n[p+"Grid"],10))B[p]++,t+=
...
l.find(".ui-datepicker-header, .ui-datepicker-calendar").hide());u=0;for(y=m.units.length;u<y;u++)p=m.units[u],r=p.substr(0,1).toUpperCase()+p.substr(1),v=null!==n["show"+r]?n["show"+r]:this.support[p],m[p+"_slider"]=m.control.create(m,w.find(".ui_tpicker_"+p+"_slider"),p,m[p],n[p+"Min"],x[p],n["step"+r]),v&&0<n[p+"Grid"]&&(t=100*B[p]*n[p+"Grid"]/(x[p]-n[p+"Min"]),w.find(".ui_tpicker_"+p+" table").css({width:t+"%",marginLeft:n.isRTL?"0":t/(-2*B[p])+"%",marginRight:n.isRTL?t/(-2*B[p])+"%":"0",borderCollapse:"collapse"}).find("td").click(function(z){var D=
...
"hour"):!1,n=this.minute_slider?this.control.value(this,this.minute_slider,"minute"):!1,m=this.second_slider?this.control.value(this,this.second_slider,"second"):!1,p=this.millisec_slider?this.control.value(this,this.millisec_slider,"millisec"):!1,r=this.microsec_slider?this.control.value(this,this.microsec_slider,"microsec"):!1,v=this.timezone_select?this.timezone_select.val():!1,x=this._defaults,B=x.pickerTimeFormat||x.timeFormat,t=x.pickerTimeSuffix||x.timeSuffix;"object"===typeof l&&(l=!1);"object"===
...
l!==(-1!==a.inArray(this.ampm.toUpperCase(),this.amNames))||null!==this.timezone&&v!==this.timezone.toString();y&&(!1!==l&&(this.hour=l),!1!==n&&(this.minute=n),!1!==m&&(this.second=m),!1!==p&&(this.millisec=p),!1!==r&&(this.microsec=r),!1!==v&&(this.timezone=v),this.inst||(this.inst=a.datepicker._getInst(this.$input[0])),this._limitMinMaxDateTime(this.inst,!0));this.support.ampm&&(this.ampm=u);this.formattedTime=a.datepicker.formatTime(x.timeFormat,this,x);this.$timeObj&&(B===x.timeFormat?this.$timeObj.val(this.formattedTime+
t):this.$timeObj.val(a.datepicker.formatTime(B,this,x)+t),this.$timeObj[0].setSelectionRange&&this.$timeObj[0].setSelectionRange(this.$timeObj[0].selectionStart,this.$timeObj[0].selectionEnd));this.timeDefined=!0;y&&this._updateDateTime()}},_onSelectHandler:function(){var l=this._defaults.onSelect||this.inst.settings.onSelect,n=this.$input?this.$input[0]:null;l&&n&&l.apply(n,[this.formattedDateTime,this])},_updateDateTime:function(l){l=this.inst||l;var n=a.datepicker._daylightSavingAdjust(0<l.currentYear?
...
this._defaults.defaultValue){this.$input.val(this._defaults.defaultValue);var l=a.datepicker._getInst(this.$input.get(0)),n=a.datepicker._get(l,"timepicker");if(n&&n._defaults.timeOnly&&l.input.val()!==l.lastVal)try{a.datepicker._updateDatepicker(l)}catch(m){a.timepicker.log(m)}}},_controls:{slider:{create:function(l,n,m,p,r,v,x){var B=l._defaults.isRTL;return n.prop("slide",null).slider({orientation:"horizontal",value:B?-1*p:p,min:B?-1*v:r,max:B?-1*r:v,step:x,slide:function(t,u){l.control.value(l,
...
0===x[v.t].length?(y="",u.ampm=""):(y=-1!==a.inArray(x[v.t].toUpperCase(),a.map(B.amNames,function(w,C){return w.toUpperCase()}))?"AM":"PM",u.ampm=B["AM"===y?"amNames":"pmNames"][0])),-1!==v.h&&(u.hour="AM"===y&&"12"===x[v.h]?0:"PM"===y&&"12"!==x[v.h]?parseInt(x[v.h],10)+12:Number(x[v.h])),-1!==v.m&&(u.minute=Number(x[v.m])),-1!==v.s&&(u.second=Number(x[v.s])),-1!==v.l&&(u.millisec=Number(x[v.l])),-1!==v.c&&(u.microsec=Number(x[v.c])),-1!==v.z&&void 0!==x[v.z]&&(u.timezone=a.timepicker.timezoneOffsetNumber(x[v.z])),
u):!1},r=function(v,x,B){try{var t=new Date("2012-01-01 "+x);if(isNaN(t.getTime())&&(t=new Date("2012-01-01T"+x),isNaN(t.getTime())&&(t=new Date("01/01/2012 "+x),isNaN(t.getTime()))))throw"Unable to parse time with native Date: "+x;return{hour:t.getHours(),minute:t.getMinutes(),second:t.getSeconds(),millisec:t.getMilliseconds(),microsec:t.getMicroseconds(),timezone:-1*t.getTimezoneOffset()}}catch(u){try{return p(v,x,B)}catch(y){a.timepicker.log("Unable to parse \ntimeString: "+x+"\ntimeFormat: "+
...
(v=n.maxDateTime),B)B.hasOwnProperty(y)&&n[y]&&(t[y]=n[y]);for(y in t)t.hasOwnProperty(y)&&(B[y]=t[y],u||=a.extend({},n),delete u[y]);if(u&&d(u))return;r?(r=0===r?new Date:new Date(r),p._defaults.minDate=r,p._defaults.minDateTime=r):v?(v=0===v?new Date:new Date(v),p._defaults.maxDate=v,p._defaults.maxDateTime=v):x&&(p._defaults.onSelect=x);if(r||v){p=a(l);var y=p.datetimepicker("getDate");l=this._base_optionDatepicker.call(a.datepicker,l,u||n,m);p.datetimepicker("setDate",y);return l}}return void 0===
...
function(l,n,m){a.timepicker.handleRange("datepicker",l,n,m)};a.timepicker.handleRange=function(l,n,m,p){function r(B,t){var u=n[l]("getDate"),y=m[l]("getDate");B=B[l]("getDate");if(null!==u){var w=new Date(u.getTime()),C=new Date(u.getTime());w.setMilliseconds(w.getMilliseconds()+p.minInterval);C.setMilliseconds(C.getMilliseconds()+p.maxInterval);if(0<p.minInterval&&w>y)m[l]("setDate",w);else if(0<p.maxInterval&&C<y)m[l]("setDate",C);else if(u>y)t[l]("setDate",B)}}function v(B,t,u){B.val()&&(B=B[l].call(B,
"getDate"),null!==B&&0<p.minInterval&&("minDate"===u&&B.setMilliseconds(B.getMilliseconds()+p.minInterval),"maxDate"===u&&B.setMilliseconds(B.getMilliseconds()-p.minInterval)),B.getTime&&t[l].call(t,"option",u,B))}p=a.extend({},{minInterval:0,maxInterval:0,start:{},end:{}},p);var x=!1;"timepicker"===l&&(x=!0,l="datetimepicker");a.fn[l].call(n,a.extend({timeOnly:x,onClose:function(B,t){r(a(this),m)},onSelect:function(B){v(a(this),m,"minDate")}},p,p.start));a.fn[l].call(m,a.extend({timeOnly:x,onClose:function(B,
...
g(r,k);else{if(l.lookupURL)p=l.lookupURL,q&&(p=p.concat(q));else if(p=S3.Ap.concat("/",l.lookupPrefix,"/",r,".json"),q){var v=r+"."+k+"="+q;p=-1!=p.indexOf("?")?p.concat("&"+v):p.concat("?"+v)}if(v=l.tooltip)v="tooltip="+v,p=-1!=p.indexOf("?")?p.concat("&"+v):p.concat("?"+v);if((v=l.represent)||"undefined"==typeof v)p=-1!=p.indexOf("?")?p.concat("&represent=1"):p.concat("?represent=1");v=null;if(l.getWidgetHTML){v=l.targetWidget||f.attr("name");var x=$('[name = "'+v+'"]');v=!0;var B=!1;if(v=x.hasClass("groupedopts-widget")?
x.groupedopts("visible"):x.data("visible")||x.is(":visible"))B=!0,x.data("visible",!0),x.hasClass("groupedopts-widget")?x.groupedopts("hide"):x.hide(),b(x,r);v=$.ajaxS3({url:p,dataType:"html",success:function(t){var u=x;if(""!==t){var y=!1,w=!1;u.hasClass("groupedopts-widget")?y=!0:u.hasClass("multiselect-widget")&&(w=!0);var C;(y||w)&&"select"==u.prop("tagName").toLowerCase()&&(C=u.val());u.html(t).trigger("change").prop("disabled",!1);if(y||w){if(C){t=[];w=0;for(var A=C.length,z;w<A;w++)z=C[w],
u.find("option[value="+z+"]").length&&t.push(z);u.val(t).trigger("change")}y?u.groupedopts("refresh"):u.multiselect("refresh")}}else u.prop("disabled",!0);x=u;B&&(x.hasClass("groupedopts-widget")?x.find("option").length&&x.groupedopts("show"):x.show());d(x,r);g(r,k,q);n||S3ClearNavigateAwayConfirm()}})}else f.each(function(){var t=$(this);(t.hasClass("groupedopts-widget")?t.groupedopts("visible"):t.data("visible")||t.is(":visible"))?(t.data("visible",!0),t.hasClass("groupedopts-widget")?t.groupedopts("hide"):
t.hide(),b(t,r)):t.data("visible",!1)}),v=$.ajaxS3({url:p,dataType:"json",success:function(t){var u=c(t,l),y=0===t.length?!0:!1;f.each(function(){var w=$(this);a:if(w.hasClass("checkboxes-widget-s3"))s3_debug("filterOptionsS3 error: checkboxes-widget-s3 not supported, updateOptions aborted"),w=void 0;else{var C=u.options,A=u.defaultValue,z=[];if(!y){var D;if(w.hasClass("checkboxes-widget-s3")){w=void 0;break a}else{(D=w.val())||(D=w.prop("value"));D||=w.data("initialValue");Array.isArray(D)||(D=D?
[D]:[]);for(var F=0,E=D.length,G;F<E;F++)(G=D[F])&&$(C).filter("option[value="+G+"]").length&&z.push(G)}z.length&&(A=z)}"INPUT"==w.prop("tagName")&&(z=$("<select>").addClass(w.attr("class")).attr("id",w.attr("id")).attr("name",w.attr("name")).data("visible",w.data("visible")).hide(),w.replaceWith(z),w=z);z=""===C;w.html(C).val(A).trigger("change").prop("disabled",z);w.hasClass("groupedopts-widget")?w.groupedopts("refresh"):w.hasClass("multiselect-widget")&&(w.multiselect("refresh"),z?w.multiselect("disable"):
...
p,v=h(v),x=x.find(n),e(x,q,v[1],f,!1)});var r="triggerUpdate."+k;p.off("change.s3options",l).on("change.s3options",l,function(){var v=h($(this));p.trigger(r,v)});p.on(r,function(v,x,B){v=("row"==f.scope?x.closest(".edit-row.inline-form,.add-row.inline-form"):p).find(n);e(v,q,B,f,!0)})}}}}})(jQuery);
...
"cluetip"));a(document).unbind(".cluetip");return this.unbind(".cluetip")}h=a.extend(!0,{},a.cluetip.defaults,h||{});g++;n=a.cluetip.backCompat||!h.multiple?"cluetip":"cluetip-"+g;var m="#"+n,p=a.cluetip.backCompat?"#":".",r=a.cluetip.setup.insertionType,v=a.cluetip.setup.insertionElement||"body";r=/appendTo|prependTo|insertBefore|insertAfter/.test(r)?r:"appendTo";var x=a(m);if(!x.length){x=a(a.cluetip.template)[r](v).attr("id",n).css({position:"absolute",display:"none"});var B=+h.cluezIndex;var t=
x.find(p+"cluetip-outer").css({position:"relative",zIndex:B});var u=x.find(p+"cluetip-inner");var y=x.find(p+"cluetip-title")}b=a("#cluetip-waitimage");b.length||(b=a("<div></div>").attr("id","cluetip-waitimage").css({position:"absolute"}));b.insertBefore(x).hide();var w=(parseInt(x.css("paddingLeft"),10)||0)+(parseInt(x.css("paddingRight"),10)||0);this.each(function(C){function A(){return!1}function z(K,T){var V=K.status;T.beforeSend(K.xhr,T);if("error"==V)T[V](K.xhr,K.textStatus);else if("success"==
...
K))return!1;I=!0;x=a(m).css({position:"absolute"});t=x.find(p+"cluetip-outer");u=x.find(p+"cluetip-inner");y=x.find(p+"cluetip-title");q=x.find(p+"cluetip-arrows");x.removeClass().css({width:ha});J==F.attr("href")&&F.css("cursor",E.cursor);E.hoverClass&&F.addClass(E.hoverClass);U=F.offset().top;ea=F.offset().left;ia=F.innerWidth();K.type==focus?(aa=ea+ia/2+S,x.css({left:X}),ca=U+R):(aa=K.pageX,ca=K.pageY);"area"!=D.tagName.toLowerCase()&&(Q=a(document).scrollTop(),ka=a(window).width());if("fixed"==
...
M;"rounded"==M&&(T+=" ui-corner-all");x.css({top:N+"px"}).attrProp({className:"cluetip ui-widget ui-widget-content ui-cluetip"+T});E.arrows?(/(left|right)/.test(W)&&(W=x.height()-q.height(),V=0<=X&&0<K?U-N-E.dropShadowSteps:0,V=(W>V?V:W)+"px"),q.css({top:V}).show()):q.hide();(l=k(x,E))&&l.length&&l.hide().css({height:L,width:ha,zIndex:F.data("cluetip").zIndex-1}).show();x.hide()[E.fx.open](E.fx.openSpeed||0);a.fn.bgiframe&&x.bgiframe();0<E.delayedClose&&(H=setTimeout(pa,E.delayedClose));E.onShow.call(D,
x,u)},ra=function(K){I=!1;b.hide();if(!E.sticky||/click|toggle/.test(E.activation))pa(),clearTimeout(H);E.hoverClass&&F.removeClass(E.hoverClass)},pa=function(K){K=K&&K.data("cluetip")?K:F;var T=K.data("cluetip")&&K.data("cluetip").selector,V=a(T||"div.cluetip"),W=V.find(p+"cluetip-inner"),na=V.find(p+"cluetip-arrows");V.hide().removeClass();E.onHide.call(K[0],V,W);T&&(K.removeClass("cluetip-clicked"),K.css("cursor",""));T&&fa&&K.attrProp(E.titleAttribute,fa);E.arrows&&na.css({top:""})};a(document).unbind("hideCluetip.cluetip").bind("hideCluetip.cluetip",
function(K){pa(a(K.target))});if(/click|toggle/.test(E.activation))F.bind("click.cluetip",function(K){x.is(":hidden")||!F.is(".cluetip-clicked")?(qa(K),a(".cluetip-clicked").removeClass("cluetip-clicked"),F.addClass("cluetip-clicked")):ra(K);return!1});else if("focus"==E.activation)F.bind("focus.cluetip",function(K){F.attrProp("title","");qa(K)}),F.bind("blur.cluetip",function(K){F.attrProp("title",F.data("cluetip").title);ra(K)});else{F[E.clickThrough?"unbind":"bind"]("click.cluetip",A);var ua=function(K){if(E.tracking){var T=
...
B+"_edit_0");"file"==u.attr("type")?(t=t.clone(),t.attr("id",u.attr("id")).attr("name",u.attr("name")).change(x),u.replaceWith(t)):(t=t.val(),u.val(t));t=a("#dummy_sub_"+c+"_"+c+"_i_"+B+"_edit_default").val();a("#dummy_sub_"+c+"_"+c+"_i_"+B+"_edit_0").val(t)}m=a("#edit-row-"+c);m.removeClass("changed");p=n._renderReadRow(c,g,p);m.hide().data("rowindex",null);p.removeClass("hide").show();n._enableAddRow();n._showHeaders()}h&&(f.hide(),e.removeClass("hide").show())},function(){h&&(f.hide(),e.removeClass("hide").show());
...
(function(){var a=function(t,u,y){if("GET"==t.type){t.type="POST";var w=function(L){var O=L[0];return"$filter"==L||"$$"==L.slice(0,2)||"_"!=O&&(-1!=L.search(/\./)||"("==O&&-1!=L.search(/\)/))},C=t.url.split("#"),A=C[0],z=null,D=null;1<C.length&&(z=C[1]);C=A.split("?");A=C[0];1<C.length&&(D=C[1]);var F=t.data,E={};C={};var G,I,H,J,M=function(L,O,P){if(Array.isArray(P))for(H=0,G=P.length;H<G;H++)M(L,O,P[H]);else L.hasOwnProperty(O)?Array.isArray(L[O])?L[O].push(P):L[O]=[L[O],P]:L[O]=P};if(F){var R=
0;for(I=F.length;R<I;R++){var S=F[R];M(C,S.name,S.value)}}if(D)for(J in F=S3.queryString.parse(D),F)D=F[J],w(J)?M(E,J,D):M(C,J,D);t.data="ajax"==u?JSON.stringify(E):E;t.processData=!1;delete C.$search;u=A+"?$search="+u;y&&(delete C.$action,u=u+"&$action="+y);(y=S3.queryString.stringify(C))&&(u+="&"+y);null!==z&&(u+="#"+z);t.url=u}};S3.search.searchRewriteAjaxOptions=a;var b={timeout:1E4,retryLimit:5,dataType:"json",contentType:"application/json; charset=utf-8",processData:!1,async:!0,type:"POST"};
...
var u,y,w=[],C,A,z,D,F,E;$(".text-filter",t).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();if(z=C.val().trim()){D=z.split(" ");var G=C.data("match"),I=[];for(u=0;u<D.length;u++)if(F=$.trim(D[u]),""!==F){var H=g("*"+F+"*");"any"==G?I.push(H):w.push([A,H])}"any"==G&&w.push([A,I.join(",")])}else w.push([A,null])});$(".s3-groupedopts-widget",t).prev(".options-filter.groupedopts-filter-widget").add($(".ui-multiselect",t).prev(".options-filter.multiselect-filter-widget")).add($(".options-filter,.options-filter.multiselect-filter-widget",
t)).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();E=$("input:radio[name='"+y+"_filter']:checked").val();switch(E){case "any":A=A.replace(/__contains$|__belongs$/,"__anyof");break;case "all":A=A.replace(/__anyof$|__belongs$/,"__contains")}if("select"==this.tagName.toLowerCase()){if(z="",D=C.val())for(D instanceof Array||(D=D.split(",")),u=0;u<D.length;u++){var G=g(D[u]);z=""===z?G:z+","+G}}else z="",$("input[name='"+y+"']:checked").each(function(){z=""===z?g($(this).val()):z+","+
g($(this).val())});""===z||"*"==z?w.push([A,null]):w.push([A,z])});$(".map-filter",t).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();(z=C.val())?w.push([A,z]):w.push([A,null])});$(".range-filter-input",t).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();(z=C.val())?w.push([A,z]):w.push([A,null])});$(".date-filter-input",t).each(function(){C=$(this);if(C.calendarWidget("instance")){y=C.attr("id");A=$("#"+y+"-data").val();z=C.val();E=y.split("-").pop();if(C.hasClass("datetimepicker"))var G=
function(Q){return Q.getFullYear()+"-"+("0"+(Q.getMonth()+1)).slice(-2)+"-"+("0"+Q.getDate()).slice(-2)+"T"+("0"+Q.getHours()).slice(-2)+":"+("0"+Q.getMinutes()).slice(-2)+":"+("0"+Q.getSeconds()).slice(-2)};else{var I="";C.hasClass("hide-time")&&(I="le"==E?"T23:59:59":"T00:00:00");G=function(Q){return Q.getFullYear()+"-"+("0"+(Q.getMonth()+1)).slice(-2)+"-"+("0"+Q.getDate()).slice(-2)+I}}var H=!1;"le"==E&&(H=!0);if(C.hasClass("negative")){if(H){var J=!1;H=y.slice(0,-3);var M=$("#"+H+"-ge");var R=
...
C.hasClass("optional")?(H=A.replace(h,""),w.push(["$filter","("+H+" "+E+' "'+O+'") or ('+H+" eq NONE)"])):w.push([A,O])):w.push([A,null])}});$(".s3-groupedopts-widget",t).prev(".location-filter.groupedopts-filter-widget").add($(".ui-multiselect",t).prev(".location-filter.multiselect-filter-widget")).add($(".location-filter,.location-filter.multiselect-filter-widget",t)).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();z="";if("select"==this.tagName.toLowerCase()){if(D=$(this).val(),
D)for(u=0;u<D.length;u++){var G=g(D[u]);z=""===z?G:z+","+G}}else $("input[name='"+y+"']:checked").each(function(){z=""===z?g($(this).val()):z+","+g($(this).val())});""===z?w.push([A,null]):w.push([A,z])});$(".hierarchy-filter",t).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();z="";if(D=C.hasClass("s3-cascade-select")?C.cascadeSelect("get"):C.hierarchicalopts("get"))for(u=0;u<D.length;u++)z=""===z?z+D[u]:z+","+D[u];""===z?w.push([A,null]):w.push([A,z])});$(".age-filter-input",t).each(function(){C=
$(this);y=C.attr("id");A=$("#"+y+"-data").val();z=C.val();""!==z?w.push([A,z]):w.push([A,null])});$(".value-filter",t).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();C.prop("checked")?w.push([A,"None"]):w.push([A,null])});w.filter(function(G){return null!=G[1]}).length?t.addClass("has-active-filters"):t.removeClass("has-active-filters");return w};S3.search.getCurrentFilters=f;S3.search.setCurrentFilters=function(t,u){void 0===t&&(t=$(".filter-form").first());t.data("noAutoSubmit",
1);var y,w={},C,A,z;var D=0;for(C=u.length;D<C;D++){var F=u[D];var E=F[0];var G="string"==typeof F[1]?c(F[1]):F[1];w.hasOwnProperty(E)?w[E]=w[E].concat(G):w[E]=G}t.find(".text-filter").each(function(){A=$(this);y=A.attr("id");E=$("#"+y+"-data").val();if(w.hasOwnProperty(E)){A.is(":visible")||A.hasClass("active")||B(t);G=w[E];z="";if(G){var I;D=0;for(C=G.length;D<C;D++)if(I=G[D])0<D&&(z+=" "),"*"==I[0]&&(I=I.slice(1)),"*"==I.slice(-1)&&(I=I.slice(0,-1)),z+=I}A.val(z)}});t.find(".options-filter").each(function(){A=
$(this);y=A.attr("id");E=$("#"+y+"-data").val();var I=$('input:radio[name="'+y+'_filter"]:checked').val();if(this.tagName&&"select"==this.tagName.toLowerCase()){var H=!1,J=E.replace(h,"");if(w.hasOwnProperty(J+"__eq"))G=w[J+"__eq"],H=!0;else if(w.hasOwnProperty(J))G=w[J],H=!0;else if(w.hasOwnProperty(E))G=w[E],H=!0;else if("any"==I||"all"==I)w.hasOwnProperty(J+"__anyof")?(G=w[J+"__anyof"],H=!0,$('input:radio[name="'+y+'_filter"][value="any"]').prop("checked",!0)):w.hasOwnProperty(J+"__contains")&&
(G=w[J+"__contains"],H=!0,$('input:radio[name="'+y+'_filter"][value="all"]').prop("checked",!0));H&&(A.is(":visible")||A.hasClass("active")||B(t),A.val(G),A.hasClass("groupedopts-filter-widget")&&A.groupedopts("instance")?A.groupedopts("refresh"):A.hasClass("multiselect-filter-widget")&&A.multiselect("instance")&&A.multiselect("refresh"))}});t.find(".map-filter").each(function(){A=$(this);y=A.attr("id");E=$("#"+y+"-data").val();if(w.hasOwnProperty(E))if(A.is(":visible")||A.hasClass("active")||B(t),
G=w[E],G){A.val(G[0]);var I=y+"-map";S3.gis.maps[I].s3.polygonButtonLoaded()}else A.val(""),I=y+"-map",S3.gis.maps[I].s3.layerRefreshed()});t.find(".range-filter-input").each(function(){A=$(this);y=A.attr("id");E=$("#"+y+"-data").val();w.hasOwnProperty(E)&&(A.is(":visible")||A.hasClass("active")||B(t),(G=w[E])?A.val(G[0]):A.val(""))});t.find(".date-filter-input").each(function(){var I=$(this),H=$("#"+I.attr("id")+"-data").val(),J=H.replace(h,""),M=!1;w.hasOwnProperty(H)?M=w[H]:w.hasOwnProperty(J)&&
(M=w[J]);!1!==M&&(I.is(":visible")||I.hasClass("active")||B(t),M?(H=M[0].replace(" ","T"),I.calendarWidget("setJSDate",new Date(H))):I.calendarWidget("clear"))});t.find(".age-filter-input").each(function(){A=$(this);y=A.attr("id");E=$("#"+y+"-data").val();w.hasOwnProperty(E)&&(A.is(":visible")||A.hasClass("active")||B(t),(G=w[E])?A.val(G[0]):A.val(""))});t.find(".location-filter").each(function(){A=$(this);y=A.attr("id");E=$("#"+y+"-data").val();var I=$('input:radio[name="'+y+'_filter"]:checked').val();
if(this.tagName&&"select"==this.tagName.toLowerCase()){var H=!1;if(w.hasOwnProperty(E))G=w[E],H=!0;else if("any"==I||"all"==I)I=E.replace(h,""),w.hasOwnProperty(I+"__anyof")?(G=w[I+"__anyof"],H=!0,$('input:radio[name="'+y+'_filter"][value="any"]').prop("checked",!0)):w.hasOwnProperty(I+"__contains")&&(G=w[I+"__contains"],H=!0,$('input:radio[name="'+y+'_filter"][value="all"]').prop("checked",!0));H&&(A.is(":visible")||A.hasClass("active")||B(t),A.val(G),A.hasClass("groupedopts-filter-widget")&&A.groupedopts("instance")?
A.groupedopts("refresh"):A.hasClass("multiselect-filter-widget")&&A.multiselect("instance")&&A.multiselect("refresh"),v(this))}});t.find(".hierarchy-filter:visible").each(function(){A=$(this);y=A.attr("id");E=$("#"+y+"-data").val();w.hasOwnProperty(E)&&(A.is(":visible")||A.hasClass("active")||B(t),G=w[E],A.hasClass("s3-cascade-select")?A.cascadeSelect("set",G):A.hierarchicalopts("set",G))});t.data("noAutoSubmit",0);t.trigger("optionChanged")};var k=function(t,u){void 0===u&&(u=f());t=t.split("?");
var y={},w={},C,A=[];u=u.filter(function(I){return"$filter"==I[0]?(A.push(I),!1):!0});A.length&&u.push(A.reduce(function(I,H){return["$filter","("+I[1]+") and ("+H[1]+")"]}));var z=0;for(C=u.length;z<C;z++){var D=u[z];var F=D[0];D=D[1];null===D?y.hasOwnProperty(F)||(w[F]=!0):(w.hasOwnProperty(F)&&(w[F]=!1),y[F]=!0)}var E=[];if(1!=S3.search.stripFilters&&1<t.length){var G=t[1].split("&");z=0;for(C=G.length;z<C;z++)D=G[z].split("="),1<D.length&&(F=S3.urlDecode(D[0]),w[F]||y[F]||E.push(G[z]))}z=0;for(C=
u.length;z<C;z++)D=u[z],F=D[0],D=D[1],y[F]&&E.push(F+"="+encodeURIComponent(D));u=E.join("&");t=t[0];u&&(t=t+"?"+u);return t};S3.search.filterURL=k;S3.search.ajaxUpdateOptions=function(t,u){var y=$(t);t=y.find("input.filter-ajax-url");t.length&&(t=$(t[0]).val());$.ajax({url:t,dataType:"json"}).done(function(w){y.data("noAutoSubmit",1);for(var C in w){var A=$("#"+C);if(A.length){var z=w[C];if(A.hasClass("options-filter")){if("select"==A[0].tagName.toLowerCase()){var D=A.siblings(".no-options-available");
...
A.hasClass("multiselect-filter-widget")&&A.multiselect("instance")?(A.multiselect("refresh"),A.multiselect("instance").$button.show()):A.removeClass("hide").show()}}}else A.hasClass("date-filter")&&(A=z.min,z=z.max,$("#"+C+"-ge").calendarWidget("instance").option("minDateTime",A).option("maxDateTime",z).refresh(),$("#"+C+"-le").calendarWidget("instance").option("minDateTime",A).option("maxDateTime",z).refresh())}}y.data("noAutoSubmit",0);u&&u.apply()}).fail(function(w,C,A){console.log("UNAUTHORIZED"==
A?i18n.gis_requires_login:w.responseText)})};var q=function(t,u){$("#"+t[0].id).closest(".dt-wrapper").find(".dt-export").each(function(){var y=$(this),w=y.data("url");w&&y.data("url",k(w,u))})},l=function(t){var u=$("#"+t).find('input.filter-submit-url[type="hidden"]').first().val(),y=d[t],w;d[t]={};for(w in y){var C=$("#"+w);var A=C.is(":visible")?!0:!1;var z=y[w];C=z.needs_reload;if(A){if(C){var D=f($("#"+t));u=k(u,D);window.location.href=u}}else d[t][w]=z}for(w in y)C=$("#"+w),C.is(":visible")&&
...
m;var p=function(t){t=$(t);if(t.hasClass("responsive"))(t=t.DataTable())&&t.responsive&&t.responsive.recalc();else if(t.hasClass("doublescroll"))try{t.dataTableS3("doubleScroll")}catch(u){}},r=function(t,u){var y=u.find(".map_wrapper"),w=y.length;w?$.when(n()).then(function(){for(var C=S3.gis,A=0;A<w;A++){var z=y[A].attributes.id.value;void 0===C.maps[z]&&C.show_map(z,C.options[z])}l(t)},function(C){s3_debug(C)},function(C){s3_debug(C)}):l(t);u.find("table.dataTable.display").each(function(){p(this)})};
S3.search.unhide_section=r;S3.search.summary_tabs=function(t,u,y){if(y)for(var w=f($("#"+t)),C=y.split(","),A=0,z=C.length;A<z;A++)y=C[A],m(t,y,w);void 0!=u?($("#summary-tabs").tabs({active:u,activate:function(D,F){D=$(F.newPanel);D.removeClass("hide");if(F.newTab.length){var E=$(F.newTab).index();F=$("#"+t).find('input.filter-submit-url[type="hidden"]');if(F.length){F=F.first();var G=$(F).val(),I=G.split("?"),H=[];if(1<I.length){G=I[1].split("&");for(var J,M,R=0;R<G.length;R++)J=G[R].split("="),
...
y=0;y<u.length;y++){var w=u[y];if(!w.hidden){var C=S3.gis;w=w.attributes.id.value;if(void 0===C.maps[w]){var A=C.options[w];void 0!=A&&(C.show_map(w,A),w=f($("#"+t)),C.refreshLayer("search_results",w))}}}};S3.search.s3map=function(t){var u=S3.gis;void 0===t&&(t="default_map");u.show_map(t,u.options[t]);t=f();u.refreshLayer("search_results",t)};var v=function(t){var u=t.name,y=u.slice(0,-1),w=parseInt(u.slice(-1)),C=$("#"+u);if(C.val()){var A=w+1,z=y+A,D=$("#"+z);D.length||(A=w+2,z=y+A,D=$("#"+z));
if(D.length){var F=z.replace(/-/g,"_");S3[F]();D.next(".ui-multiselect").show();F=S3.location_filter_hierarchy;if(void 0!=S3.location_name_l10n){D=!0;var E=S3.location_name_l10n}else D=!1;t.options1=[];t.options2=[];t.options3=[];t.options4=[];t.options5=[];for(u=w;0<=u&&!F.hasOwnProperty("L"+u);u--);var G=function(L){var O,P;for(O in L)if(L.hasOwnProperty(O)){O&&"null"!=O&&t["options"+A].push(O);var Q=L[O];if("object"===typeof Q&&(z=A+1,$("#"+y+z).length||(z=A+2),$("#"+y+z).length))for(P in Q)Q.hasOwnProperty(P)&&
P&&"null"!=P&&t["options"+z].push(P)}},I=function(L,O){var P,Q,U=$("#"+y+O).val();for(Q in L)if(L.hasOwnProperty(Q))if(null===U)if(O===w)G(L[Q]);else{var N=O+1;$("#"+y+N).length||N++;I(L[Q],N)}else for(P in U)U[P]===Q&&(O===w?G(L[Q]):(N=O+1,$("#"+y+N).length||N++,I(L[Q],N)))};I(F["L"+u],u);for(F=A;5>=F;F++){var H=$("#"+y+F);if(H.length){var J=t["options"+F];J.sort();var M="";for(var R in J)if(J.hasOwnProperty(R)){u=J[R];var S=D?E[u]||u:u;M+='<option value="'+u+'">'+S+"</option>"}H.html(M);if(H.hasClass("groupedopts-filter-widget")&&
...
u.length&&(u.hide(),u=u.next(".location-filter").next(".ui-multiselect"),u.length&&u.hide())))))}}}}else{for(F=w+1;5>=F;F++)if(H=$("#"+y+F),H.length)if(H.html(""),H.hasClass("groupedopts-filter-widget")&&H.groupedopts("instance"))try{H.groupedopts("refresh")}catch(L){}else H.hasClass("multiselect-filter-widget")&&H.multiselect("instance")&&H.multiselect("refresh");u=C.next(".ui-multiselect").next(".location-filter").next(".ui-multiselect");u.length&&(u.hide(),u=u.next(".location-filter").next(".ui-multiselect"),
u.length&&(u.hide(),u=u.next(".location-filter").next(".ui-multiselect"),u.length&&(u.hide(),u=u.next(".location-filter").next(".ui-multiselect"),u.length&&(u.hide(),u=u.next(".location-filter").next(".ui-multiselect"),u.length&&u.hide()))))}},x=function(t){S3.hideAlerts("warning");var u=t.attr("id"),y=t.find('input.filter-submit-url[type="hidden"]').val(),w=f(t);if(t.hasClass("filter-ajax")){t=t.find('input.filter-submit-target[type="hidden"]').val();d[u]={};t=t.split(" ");var C={},A;for(A=0;A<t.length;A++){var z=
t[A];var D=$("#"+z);var F=D.is(":visible")?!0:!1;var E=!1;var G=null;D.hasClass("dataTable")?(E=$("input#"+t[A]+"_configurations"),E.length&&(G=JSON.parse($(E).val()),G=G.ajaxUrl,G="undefined"!=typeof G?k(G,w):null),G?(C[t[A]]=G,E=!1):E=!0):D.hasClass("dl")||D.hasClass("map_wrapper")||D.hasClass("cms_content")||D.hasClass("gi-container")||D.hasClass("pt-container")||D.hasClass("tp-container")||D.hasClass("s3-target")||D.hasClass("s3-organizer")?E=!1:F?(y=k(y,w),window.location.href=y):E=!0;F||(d[u][z]=
...
function(){$(".filter-manager-row").removeClass("hide").show();$(".show-filter-manager").hide()});$(".filter-submit").on("click",function(){x($(this).closest(".filter-form"))});$(".filter-form").on("dataChanged",function(t,u,y){u&&(t=$(this).find("input.filter-submit-target").val())&&(t=t.split(" "),t.length&&-1!=$.inArray(u+"",t)&&S3.search.ajaxUpdateOptions(this,y));return!1});$(".filter-advanced").on("click",function(){B($(this).closest("form"))});$(".location-filter").on("change",function(){v(this)});
...
function(){$(this).closest("form").trigger("optionChanged")});$(".hierarchy-filter").on("select.s3hierarchy",function(){$(this).closest("form").trigger("optionChanged")});$(".map-filter").each(function(){var t=$(this);$.when(n()).then(function(){var u=t.attr("id"),y=$("#"+u),w,C=S3.gis,A=C.maps[u+"-map"],z=A.s3;z.pointPlaced=function(D){var F={internalProjection:A.getProjectionObject(),externalProjection:C.proj4326};w=(new OpenLayers.Format.WKT(F)).write(D);y.val('"'+w+'"').trigger("change")};z.polygonButtonOff=
function(){y.val("").trigger("change")};z.layerRefreshed=function(){var D=z.polygonButton;D&&"drawpolygonclear-off"==D.getIconClass()&&(z.lastDraftFeature?z.lastDraftFeature.destroy():1<z.draftLayer.features.length&&z.draftLayer.features[0].destroy(),D.control.deactivate(),D.items[0].pressed=!0)};z.polygonButtonLoaded=function(){if(w=y.val()){z.polygonButton.items[0].btnEl.dom.click();var D=OpenLayers.Geometry.fromWKT(w);D.transform(C.proj4326,A.getProjectionObject());D=new OpenLayers.Feature.Vector(D,
...
(function(){S3.autocomplete={};S3.autocomplete.normal=function(d,g,c,e,h,f,k,q,l){var n="dummy_"+e,m=$("#"+n);if(m.length){g=S3.Ap.concat("/",g,"/",c,"/search_ac.json?field=",d);h&&(g+="&"+h);f&&(g+="&link="+f);var p=$("#"+e).hide();e=p.val();var r;e&&(r={value:e,label:m.val()});p.data("existing",r);p.data("url",g);var v=$("#"+n+"_throbber");void 0==q&&(q=800);void 0==l&&(l=2);m.val();m.autocomplete({delay:q,minLength:l,source:function(x,B){$.ajax({url:p.data("url"),data:{term:x.term}}).done(function(t){var u=
{id:0};0==t.length?(p.val("").trigger("change"),u[d]=i18n.no_matching_records):u[d]=i18n.none_of_the_above;t.push(u);B(t)})},search:function(x,B){v.removeClass("hide").show();return!0},response:function(x,B,t){v.hide();return t},focus:function(x,B){return!1},select:function(x,B){x=B.item;x.id?(m.val(x[d]),p.val(x.id).trigger("change"),r={value:x.id,label:x[d]}):(m.val(""),p.val("").trigger("change"));k&&eval(k);return!1}}).data("ui-autocomplete")._renderItem=function(x,B){var t=B.label?B.label:B[d];
return $("<li>").data("item.autocomplete",B).append("<a>"+t+"</a>").appendTo(x)};m.on("blur",function(){r&&r.label!=m.val()&&p.val("").trigger("change")})}};S3.autocomplete.generic=function(d,g,c,e,h){var f="dummy_"+g,k=$("#"+f);if(k.length){var q=$("#"+g).hide();g=q.val();var l;g&&(l={value:g,name:k.val()});q.data("existing",l);q.data("url",d);var n=$("#"+f+"_throbber");void 0==e&&(e=800);void 0==h&&(h=2);k.autocomplete({delay:e,minLength:h,source:function(m,p){$.ajax({url:q.data("url"),data:{term:m.term}}).done(function(r){0==
...
label:i18n.none_of_the_above});x(B)})},search:function(v,x){r.removeClass("hide").show();return!0},response:function(v,x,B){r.hide();return B},focus:function(v,x){return!1},select:function(v,x){v=x.item;v.id?(l.val(v.name),m.val(v.id).trigger("change"),p={value:v.id,name:v.name}):(l.val(""),m.val("").trigger("change"));e&&eval(e);return!1}}).data("ui-autocomplete")._renderItem=function(v,x){var B=x.label?x.label:x.name;return $("<li>").data("item.autocomplete",x).append("<a>"+B+"</a>").appendTo(v)};
...
(r+=" ("+v,x&&(r+=", "+x),r+=")"):r+=" ("+x+")";var B=p.org;x=p.instance_type;if(B||x)x?(r+=" ("+S3.org_site_types[x],B&&(r+=", "+B),r+=")"):r+=" ("+B+")"}else if("addr"==p.match_type){if(r=p.name+" ("+r,r=(x=p.L4||p.L3||p.L2||p.L1)?r+(", "+x+")"):r+")",B=p.org,x=p.instance_type,B||x)x?(r+=" ("+S3.org_site_types[x],B&&(r+=", "+B),r+=")"):r+=" ("+B+")"}else if("org"==p.match_type){B=p.name;v=p.addr;x=p.L4||p.L3||p.L2||p.L1;if(v||x)v?(B+=" ("+v,x&&(B+=", "+x),B+=")"):B+=" ("+x+")";r=(x=p.instance_type)?
B+" ("+S3.org_site_types[x]+", "+r+")":B+" ("+r+")"}else{if(B=p.name,r=(v=p.addr)?B+" ("+v+", "+r+")":B+" ("+r+")",B=p.org,x=p.instance_type,B||x)x?(r+=" ("+S3.org_site_types[x],B&&(r+=", "+B),r+=")"):r+=" ("+B+")"}else r=p.name;return $("<li>").data("item.autocomplete",p).append("<a>"+r+"</a>").appendTo(m)};f.on("blur",function(){l&&l.name!=f.val()&&q.val("").trigger("change")})}}})();
