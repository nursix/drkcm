...
t.hide(),b(t,r)):t.data("visible",!1)}),v=$.ajaxS3({url:p,dataType:"json",success:function(t){var u=c(t,l),y=0===t.length?!0:!1;f.each(function(){var w=$(this);a:if(w.hasClass("checkboxes-widget-s3"))s3_debug("filterOptionsS3 error: checkboxes-widget-s3 not supported, updateOptions aborted"),w=void 0;else{var C=u.options,A=u.defaultValue,z=[];if(!y){var D;if(w.hasClass("checkboxes-widget-s3")){w=void 0;break a}else{(D=w.val())||(D=w.prop("value"));D||=w.data("selected");Array.isArray(D)||(D=D?[D]:
[]);for(var F=0,E=D.length,G;F<E;F++)(G=D[F])&&$(C).filter("option[value="+G+"]").length&&z.push(G)}z.length&&(A=z)}"INPUT"==w.prop("tagName")&&(z=$("<select>").addClass(w.attr("class")).attr("id",w.attr("id")).attr("name",w.attr("name")).data("visible",w.data("visible")).hide(),w.replaceWith(z),w=z);z=""===C;w.html(C).val(A).trigger("change").prop("disabled",z);w.hasClass("groupedopts-widget")?w.groupedopts("refresh"):w.hasClass("multiselect-widget")&&(w.multiselect("refresh"),z?w.multiselect("disable"):
