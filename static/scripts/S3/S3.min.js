...
return}}else m=!0;if(!m&&!n){var p=f.val();void 0==f.data("initialValue")&&f.data("initialValue",p)}m&&l.getWidgetHTML&&(s3_debug("filterOptionsS3 warning: getWidgetHTML=true not suitable for multiple target widgets (e.g. inline rows)"),f=f.first(),m=!1);m=m?f.first():f;if(p=m.data("update-request"))try{p.abort()}catch(t){}var r=l.lookupResource;if(""===q||null===q||void 0===q)f.val("").prop("disabled",!0),f.multiselect("instance")&&f.multiselect("refresh").multiselect("disable"),f.trigger("change"),
g(r,k);else{if(l.lookupURL)p=l.lookupURL,q&&(p=p.concat(q));else if(p=S3.Ap.concat("/",l.lookupPrefix,"/",r,".json"),q){var v=r+"."+k+"="+q;p=-1!=p.indexOf("?")?p.concat("&"+v):p.concat("?"+v)}if(v=l.tooltip)v="tooltip="+v,p=-1!=p.indexOf("?")?p.concat("&"+v):p.concat("?"+v);if((v=l.represent)||"undefined"==typeof v)p=-1!=p.indexOf("?")?p.concat("&represent=1"):p.concat("?represent=1");v=null;if(l.getWidgetHTML){v=l.targetWidget||f.attr("name");var x=$('[name = "'+v+'"]');v=!0;var A=!1;if(v=x.hasClass("groupedopts-widget")?
x.groupedopts("visible"):x.data("visible")||x.is(":visible"))A=!0,x.data("visible",!0),x.hasClass("groupedopts-widget")?x.groupedopts("hide"):x.hide(),b(x,r);v=$.ajaxS3({url:p,dataType:"html",success:function(t){var u=x;if(""!==t){var y=!1,w=!1;u.hasClass("groupedopts-widget")?y=!0:u.hasClass("multiselect-widget")&&(w=!0);var D;(y||w)&&"select"==u.prop("tagName").toLowerCase()&&(D=u.val());u.html(t).trigger("change").prop("disabled",!1);if(y||w){if(D){t=[];w=0;for(var B=D.length,z;w<B;w++)z=D[w],
u.find("option[value="+z+"]").length&&t.push(z);u.val(t).trigger("change")}y?u.groupedopts("refresh"):u.multiselect("refresh")}}else u.prop("disabled",!0);x=u;A&&(x.hasClass("groupedopts-widget")?x.find("option").length&&x.groupedopts("show"):x.show());d(x,r);g(r,k,q);n||S3ClearNavigateAwayConfirm()}})}else f.each(function(){var t=$(this);(t.hasClass("groupedopts-widget")?t.groupedopts("visible"):t.data("visible")||t.is(":visible"))?(t.data("visible",!0),t.hasClass("groupedopts-widget")?t.groupedopts("hide"):
t.hide(),b(t,r)):t.data("visible",!1)}),v=$.ajaxS3({url:p,dataType:"json",success:function(t){var u=c(t,l),y=0===t.length?!0:!1;f.each(function(){var w=$(this);a:if(w.hasClass("checkboxes-widget-s3"))s3_debug("filterOptionsS3 error: checkboxes-widget-s3 not supported, updateOptions aborted"),w=void 0;else{var D=u.options,B=u.defaultValue,z=[];if(!y){var C;if(w.hasClass("checkboxes-widget-s3")){w=void 0;break a}else{(C=w.val())||(C=w.prop("value"));C||=w.data("initialValue");Array.isArray(C)||(C=C?
[C]:[]);for(var F=0,E=C.length,G;F<E;F++)(G=C[F])&&$(D).filter("option[value="+G+"]").length&&z.push(G)}z.length&&(B=z)}"INPUT"==w.prop("tagName")&&(z=$("<select>").addClass(w.attr("class")).attr("id",w.attr("id")).attr("name",w.attr("name")).data("visible",w.data("visible")).hide(),w.replaceWith(z),w=z);z=""===D;w.html(D).val(B).trigger("change").prop("disabled",z);w.hasClass("groupedopts-widget")?w.groupedopts("refresh"):w.hasClass("multiselect-widget")&&(w.multiselect("refresh"),z?w.multiselect("disable"):
w.multiselect("enable"))}w.data("visible")&&(w.hasClass("groupedopts-widget")?y||w.groupedopts("show"):w.show());d(w,r)});g(r,k,q);n||S3ClearNavigateAwayConfirm()}});m.data("update-request",v)}},h=function(f){var k="";if("checkbox"==f.attr("type")){var q=f.closest(".checkboxes-widget-s3");q&&(f=q)}if(f.hasClass("checkboxes-widget-s3"))k=[],f.find("input:checked").each(function(){k.push($(this).val())});else if(f.hasClass("s3-hierarchy-input")){if(k="",q=f.val())q=JSON.parse(q),q.constructor===Array?
q.length&&(k=q[0]):q&&(k=q)}else 1==f.length&&(k=f.val());return[f,k]};$.filterOptionsS3=function(f){var k=f.trigger;k=f.event?f.event:"string"==typeof k?k:k.name;var q=f.lookupKey||k,l=a(f.trigger),n=a(f.target);if(n){var m=$(n);if(m.length){var p=m.closest("form");if(l&&(m=p.find(l),m.length)){$(l).each(function(){var v=$(this);var x=v.closest(".inline-form");if(!x.length||!x.hasClass("empty-row")&&"none"!=x.css("display"))x="row"==f.scope?v.closest(".edit-row.inline-form,.add-row.inline-form"):
p,v=h(v),x=x.find(n),e(x,q,v[1],f,!1)});var r="triggerUpdate."+k;p.off("change.s3options",l).on("change.s3options",l,function(){var v=h($(this));p.trigger(r,v)});p.on(r,function(v,x,A){v=("row"==f.scope?x.closest(".edit-row.inline-form,.add-row.inline-form"):p).find(n);e(v,q,A,f,!0)})}}}}})(jQuery);
...
jQuery.fn.labelOver=function(a){return $(this).each(function(){var b=jQuery(this),d=b.attr("for");if(d){var g=jQuery("#"+d);this.hide=function(){b.css({textIndent:-1E4})};this.show=function(){""==g.val()&&b.css({textIndent:0})};g.focus(this.hide);g.blur(this.show);b.addClass(a).click(function(){g.focus()});""!=g.val()&&this.hide()}})};
...
return this._get_days_in_month(b.getFullYear(),b.getMonth())},_add_months:function(b,d){a(this.element);var g=b.getDate();b.setDate(1);b.setMonth(b.getMonth()+d);b.setDate(Math.min(g,this._get_days_in_month_big(b)));return b}})})(jQuery);
...
var q;if(a("#add-row-"+g).hasClass("required")){var l=!0;for(q in k)"_id"!=q&&""!==k[q]&&(l=!1);if(l&&a("#add-"+g+"-none").length&&a("#read-row-"+g+"-0").length)return h.hide(),c.removeClass("hide").show(),a.Deferred().resolve()}var n=this._validate(f,"none",k),m=this;return n.then(function(p){if(null!==p){p._changed=!0;var r=f.data.push(p)-1;p._index=r;m._serialize();if(e){for(var v=[],x=f.fields,A,t,u,y=0,w=x.length;y<w;y++)A=x[y].name,t=a("#upload_"+g+"_"+A+"_none"),t.length&&(u="upload_"+g+"_"+
A+"_"+r,a("#"+u).remove(),t.attr({id:u,name:u})),v.push(p[A].text);m._resetAddRow();p=m._renderReadRow(g,r,v);m._appendReadRow(g,p);m._showHeaders()}}e&&(h.hide(),c.removeClass("hide").show())},function(){e&&(h.hide(),c.removeClass("hide").show());return n})},_updateRow:function(g){var c=this.formname,e=a("#rdy-"+c+"-0"),h;if(h=e.length?!0:!1){e.hide();var f=a("#throbber-"+c+"-0").removeClass("hide").show();this._removeErrors()}var k=this._deserialize(),q=this._collectData(k,"0",g);if(null===q)return h&&
(f.hide(),e.removeClass("hide").show()),a.Deferred().reject();if(q._delete)return k.data[g]._delete=!0,this._serialize(),a.Deferred().resolve();var l=this._validate(k,"0",q),n=this;return l.then(function(m){if(null!==m&&(m._id=k.data[g]._id,m._changed=!0,m._index=g,k.data[g]=m,n._serialize(),h)){var p=[],r=k.fields,v,x=function(){n._markChanged(n);n._catchSubmit()};for(v=0;v<r.length;v++){var A=r[v].name;p.push(m[A].text);var t=a("#sub_"+c+"_"+c+"_i_"+A+"_edit_default");var u=a("#sub_"+c+"_"+c+"_i_"+
A+"_edit_0");"file"==u.attr("type")?(t=t.clone(),t.attr("id",u.attr("id")).attr("name",u.attr("name")).change(x),u.replaceWith(t)):(t=t.val(),u.val(t));t=a("#dummy_sub_"+c+"_"+c+"_i_"+A+"_edit_default").val();a("#dummy_sub_"+c+"_"+c+"_i_"+A+"_edit_0").val(t)}m=a("#edit-row-"+c);m.removeClass("changed");p=n._renderReadRow(c,g,p);m.hide().data("rowindex",null);p.removeClass("hide").show();n._enableAddRow();n._showHeaders()}h&&(f.hide(),e.removeClass("hide").show())},function(){h&&(f.hide(),e.removeClass("hide").show());
...
g.off(c);return!0}})})(jQuery);$(function(){$(".inline-component").inlinecomponent(S3.inlineComponentsOpts||{})});
...
A+" ("+S3.org_site_types[x]+", "+r+")":A+" ("+r+")"}else{if(A=p.name,r=(v=p.addr)?A+" ("+v+", "+r+")":A+" ("+r+")",A=p.org,x=p.instance_type,A||x)x?(r+=" ("+S3.org_site_types[x],A&&(r+=", "+A),r+=")"):r+=" ("+A+")"}else r=p.name;return $("<li>").data("item.autocomplete",p).append("<a>"+r+"</a>").appendTo(m)};f.on("blur",function(){l&&l.name!=f.val()&&q.val("").trigger("change")})}}})();S3.supply={};S3.supply.fncPrepItem=function(a){for(var b=0;b<a.length;b++)if(1==a[b].quantity)return a[b].name;return""};
S3.supply.fncRepresentItem=function(a,b){return 1==a.quantity?a.name:a.name+" ("+a.quantity+" x "+b+")"};
$(function(){var a=function(){var b=$("#inv_recv_type").val();void 0!=b&&(11==b?($('[id^="inv_recv_from_site_id__row"]').show(),$('[id^="inv_recv_organisation_id__row"]').hide()):32<=b?($('[id^="inv_recv_from_site_id__row"]').hide(),$('[id^="inv_recv_organisation_id__row"]').show()):($('[id^="inv_recv_from_site_id__row"]').hide(),$('[id^="inv_recv_organisation_id__row"]').hide()))};$("#inv_track_item_item_pack_id").change(function(){try{S3.JSONRequest[$(this).attr("id")].abort()}catch(d){}$("#TotalQuantity").remove();if(0<
$('[name="inv_item_id"]').length)id=$('[name="inv_item_id"]').val();else if(0<$('[name="send_inv_item_id"]').length)id=$('[name="send_inv_item_id"]').val();else return;var b=S3.Ap.concat("/inv/inv_item_quantity.json/"+id);0===$("#inv_quantity_throbber").length&&$('[name="quantity"]').after('<div id="inv_quantity_throbber" class="throbber">');S3.JSONRequest[$(this).attr("id")]=$.getJSON(b,function(d){var g=d.iquantity;d=d.pquantity;var c=$('[name="item_pack_id"] option:selected').text(),e=/\(([0-9]*)\sx/.exec(c);
...
RecvTable+=" - </a>"),RecvTable+="</td><td>"+e[i].quantity+"</td></tr>";RecvTable+="</table>";$(".quantity_req_ajax_throbber",this.parent()).remove();this.parent().after(RecvTable)})}else b.removeClass("expanded").addClass("collapsed"),$(".recv_table",b.parent().parent()).remove()})});S3.search={};
