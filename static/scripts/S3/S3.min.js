...
u.find("option[value="+z+"]").length&&t.push(z);u.val(t).trigger("change")}y?u.groupedopts("refresh"):u.multiselect("refresh")}}else u.prop("disabled",!0);x=u;B&&(x.hasClass("groupedopts-widget")?x.find("option").length&&x.groupedopts("show"):x.show());d(x,r);g(r,k,q);n||S3ClearNavigateAwayConfirm()}})}else f.each(function(){var t=$(this);(t.hasClass("groupedopts-widget")?t.groupedopts("instance")&&t.groupedopts("visible"):t.hasClass("multiselect-widget")?t.multiselect("instance")&&t.multiselect("visible"):
t.data("visible")||t.is(":visible"))?(t.data("visible",!0),t.hasClass("groupedopts-widget")?t.groupedopts("hide"):t.hasClass("groupedopts-widget")?t.multiselect("hide"):t.hide(),b(t,r)):t.data("visible",!1)}),v=$.ajaxS3({url:p,dataType:"json",success:function(t){var u=c(t,l),y=0===t.length?!0:!1;f.each(function(){var w=$(this);a:if(w.hasClass("checkboxes-widget-s3"))s3_debug("filterOptionsS3 error: checkboxes-widget-s3 not supported, updateOptions aborted"),w=void 0;else{var C=u.options,A=u.defaultValue,
z=[];if(!y){var D;if(w.hasClass("checkboxes-widget-s3")){w=void 0;break a}else{(D=w.val())||(D=w.prop("value"));D||=w.data("selected");Array.isArray(D)||(D=D?[D]:[]);for(var F=0,E=D.length,G;F<E;F++)(G=D[F])&&$(C).filter("option[value="+G+"]").length&&z.push(G)}z.length&&(A=z)}"INPUT"==w.prop("tagName")&&(z=$("<select>").addClass(w.attr("class")).attr("id",w.attr("id")).attr("name",w.attr("name")).data("visible",w.data("visible")).hide(),w.replaceWith(z),w=z);z=""===C;w.html(C).val(A).trigger("change").prop("disabled",
z);w.hasClass("groupedopts-widget")?w.groupedopts("refresh"):w.hasClass("multiselect-widget")&&(w.multiselect("refresh"),z?w.multiselect("disable"):w.multiselect("enable"))}w.data("visible")&&(w.hasClass("groupedopts-widget")?y||w.groupedopts("show"):w.show());d(w,r)});g(r,k,q);n||S3ClearNavigateAwayConfirm()}});m.data("update-request",v)}},h=function(f){var k="";if("checkbox"==f.attr("type")){var q=f.closest(".checkboxes-widget-s3");q&&(f=q)}if(f.hasClass("checkboxes-widget-s3"))k=[],f.find("input:checked").each(function(){k.push($(this).val())});
else if(f.hasClass("s3-hierarchy-input")){if(k="",q=f.val())q=JSON.parse(q),q.constructor===Array?q.length&&(k=q[0]):q&&(k=q)}else 1==f.length&&(k=f.val());return[f,k]};$.filterOptionsS3=function(f){var k=f.trigger;k=f.event?f.event:"string"==typeof k?k:k.name;var q=f.lookupKey||k,l=a(f.trigger),n=a(f.target);if(n){var m=$(n);if(m.length){void 0!==m.attr("multiple")&&(f.multiple=!0);var p=m.closest("form");if(l&&(m=p.find(l),m.length)){$(l).each(function(){var v=$(this);var x=v.closest(".inline-form");
if(!x.length||!x.hasClass("empty-row")&&"none"!=x.css("display"))x="row"==f.scope?v.closest(".edit-row.inline-form,.add-row.inline-form"):p,v=h(v),x=x.find(n),e(x,q,v[1],f,!1)});var r="triggerUpdate."+k;p.off("change.s3options",l).on("change.s3options",l,function(){var v=h($(this));p.trigger(r,v)});p.on(r,function(v,x,B){v=("row"==f.scope?x.closest(".edit-row.inline-form,.add-row.inline-form"):p).find(n);e(v,q,B,f,!0)})}}}}})(jQuery);
...
B+" ("+S3.org_site_types[x]+", "+r+")":B+" ("+r+")"}else{if(B=p.name,r=(v=p.addr)?B+" ("+v+", "+r+")":B+" ("+r+")",B=p.org,x=p.instance_type,B||x)x?(r+=" ("+S3.org_site_types[x],B&&(r+=", "+B),r+=")"):r+=" ("+B+")"}else r=p.name;return $("<li>").data("item.autocomplete",p).append("<a>"+r+"</a>").appendTo(m)};f.on("blur",function(){l&&l.name!=f.val()&&q.val("").trigger("change")})}}})();S3.supply={};S3.supply.fncPrepItem=function(a){for(var b=0;b<a.length;b++)if(1==a[b].quantity)return a[b].name;return""};
S3.supply.fncRepresentItem=function(a,b){return 1==a.quantity?a.name:a.name+" ("+a.quantity+" "+b+")"};
...
typeof t&&(t=$(".filter-form").first());var u,y,w=[],C,A,z,D,F,E;$(".text-filter",t).each(function(){C=$(this);y=C.attr("id");A=$("#"+y+"-data").val();if(z=C.val().trim()){D=z.split(" ");var G=C.data("match"),I=[];for(u=0;u<D.length;u++)if(F=$.trim(D[u]),""!==F){var H=g("*"+F+"*");"any"==G?I.push(H):w.push([A,H])}"any"==G&&w.push([A,I.join(",")])}else w.push([A,null])});$(".s3-groupedopts-widget",t).prev(".options-filter.groupedopts-filter-widget").add($(".ui-multiselect",t).prev(".options-filter.multiselect-filter-widget")).add($(".options-filter,.options-filter.multiselect-filter-widget",
...
this._showCRUDButtons()},_load:function(){const g=a(this.element).closest("form"),c=this.options.filters;S3.search.clearFilters(g);let e=a(el).val();e&&c.hasOwnProperty(e)&&S3.search.setCurrentFilters(g,c[e])},clear:function(){a(this.element).val("");this._cancel()},_showCRUDButtons:function(){var g=this.options;this._hideCRUDButtons();!g.readOnly&&g.allowCreate&&this.create_btn.show();a(this.element).val()&&(g.explicitLoad&&this.load_btn.show(),g.readOnly||(g.allowUpdate&&this.save_btn.show(),g.allowDelete&&
this.delete_btn.show()))},_hideCRUDButtons:function(){this.create_btn.hide();this.load_btn.hide();this.delete_btn.hide();this.save_btn.hide()},_getFilterURL:function(){var g=a(this.element).closest("form").find('input.filter-submit-url[type="hidden"]');return g.length?g.first().val():document.URL},_bindEvents:function(){var g=this;this.create_btn.on("click",function(){g._newFilter()});this.accept_btn.on("click",function(){g._accept()});this.cancel_btn.on("click",function(){g._cancel()});this.element.on("change",
function(){g._showCRUDButtons();g.options.explicitLoad||g._load()});if(this.options.explicitLoad)this.load_btn.on("click",function(){g._load()});this.save_btn.on("click",function(){g._save()});this.delete_btn.on("click",function(){g._delete()})},_unbindEvents:function(){this.create_btn&&this.create_btn.off("click");this.accept_btn&&this.accept_btn.off("click");this.cancel_btn&&this.cancel_btn.off("click");this.load_btn&&this.options.explicitLoad&&this.load_btn.off("click");this.save_btn&&this.save_btn.off("click");
this.delete_btn&&this.delete_btn.off("click");this.element.off("change")}})})(jQuery);
