...
p,r){n.hide();return r},focus:function(m,p){return!1},select:function(m,p){m=p.item;m.id?(f.val(m.name),q.val(m.id).trigger("change"),l={value:m.id,name:m.name}):(f.val(""),q.val("").trigger("change"));g&&eval(g);return!1}}).data("ui-autocomplete")._renderItem=function(m,p){if(void 0!=p.label)var r=p.label;else if(p.match_type)if(r="<b>"+p.match_string+"</b>",p.pre_string&&(r=p.pre_string+r),p.next_string&&(r+=p.next_string),"name"==p.match_type){var v=p.addr;var x=p.L4||p.L3||p.L2||p.L1;if(v||x)v?
(r+=" ("+v,x&&(r+=", "+x),r+=")"):r+=" ("+x+")";var A=p.org;x=p.instance_type;if(A||x)x?(r+=" ("+S3.org_site_types[x],A&&(r+=", "+A),r+=")"):r+=" ("+A+")"}else if("addr"==p.match_type){if(r=p.name+" ("+r,r=(x=p.L4||p.L3||p.L2||p.L1)?r+(", "+x+")"):r+")",A=p.org,x=p.instance_type,A||x)x?(r+=" ("+S3.org_site_types[x],A&&(r+=", "+A),r+=")"):r+=" ("+A+")"}else if("org"==p.match_type){A=p.name;v=p.addr;x=p.L4||p.L3||p.L2||p.L1;if(v||x)v?(A+=" ("+v,x&&(A+=", "+x),A+=")"):A+=" ("+x+")";r=(x=p.instance_type)?
A+" ("+S3.org_site_types[x]+", "+r+")":A+" ("+r+")"}else{if(A=p.name,r=(v=p.addr)?A+" ("+v+", "+r+")":A+" ("+r+")",A=p.org,x=p.instance_type,A||x)x?(r+=" ("+S3.org_site_types[x],A&&(r+=", "+A),r+=")"):r+=" ("+A+")"}else r=p.name;return $("<li>").data("item.autocomplete",p).append("<a>"+r+"</a>").appendTo(m)};f.on("blur",function(){l&&l.name!=f.val()&&q.val("").trigger("change")})}}})();
