...
(B.push(H),!1):!0});B.length&&u.push(B.reduce(function(H,I){return["$filter","("+H[1]+") and ("+I[1]+")"]}));var z=0;for(D=u.length;z<D;z++){var C=u[z];var F=C[0];C=C[1];null===C?y.hasOwnProperty(F)||(w[F]=!0):(w.hasOwnProperty(F)&&(w[F]=!1),y[F]=!0)}var E=[];if(1!=S3.search.stripFilters&&1<t.length){var G=t[1].split("&");z=0;for(D=G.length;z<D;z++)C=G[z].split("="),1<C.length&&(F=S3.urlDecode(C[0]),w[F]||y[F]||E.push(G[z]))}z=0;for(D=u.length;z<D;z++)C=u[z],F=C[0],C=C[1],y[F]&&E.push(F+"="+encodeURIComponent(C));
u=E.join("&");t=t[0];u&&(t=t+"?"+u);return t};S3.search.filterURL=k;S3.search.ajaxUpdateOptions=function(t,u){var y=$(t);t=y.find("input.filter-ajax-url");t.length&&(t=$(t[0]).val());$.ajax({url:t,dataType:"json"}).done(function(w){y.data("noAutoSubmit",1);for(var D in w){var B=$("#"+D);if(B.length){var z=w[D];if(B.hasClass("options-filter")){if("select"==B[0].tagName.toLowerCase()){var C=B.siblings(".no-options-available");if(z.hasOwnProperty("empty"))B.html(""),B.hasClass("multiselect-filter-widget")&&
...
activate:function(C,F){C=$(F.newPanel);C.removeClass("hide");if(F.newTab.length){var E=$(F.newTab).index();F=$("#"+t).find('input.filter-submit-url[type="hidden"]');if(F.length){F=F.first();var G=$(F).val(),H=G.split("?"),I=[];if(1<H.length){G=H[1].split("&");for(var K,M,S=0;S<G.length;S++)K=G[S].split("="),1<K.length&&(M=S3.urlDecode(K[0]),"t"!=M&&I.push(K[0]+"="+K[1]));I.push("t="+E);E=I.join("&");H=H[0];E&&(H=H+"?"+E)}else H=G+"?t="+E;$(F).val(H)}}r(t,C)}}).css({visibility:"visible"}),$('.ui-tabs-panel[aria-hidden="false"]').first().removeClass("hide").find("table.dataTable.display").each(function(){p(this)})):
