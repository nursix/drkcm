...
var f=this.options,b=f.access,c=f.personID,m=f.controller;a=S3.Ap.concat("/pr/contact/create.iframe?person="+c);b&&(a+="&access="+b);d.get(a,function(k){var h=d('<div class="iframe-container">').hide().html(k);k=S3.Ap.concat("/pr/contact/create?person="+c+"&controller="+m);b&&(k+="&access="+b);var g=h.find("form").attr("action",k);k=d('<a class="cancel action-lnk">'+f.cancelButtonText+"</a>").on("click",function(){g.slideUp("fast",function(){h.remove()});e.show()});g.find('input[type="submit"]').after(k);
h.insertAfter(e).slideDown("fast",function(){e.siblings(".throbber").hide();h.find("#popup").show();g.show()})})},_addEmergencyContact:function(a){var e=d(a);d(".iframe-container").remove();this._showAll();e.hide().siblings(".throbber").removeClass("hide").show();var f=this.options,b=f.access,c=f.personID,m=f.controller,k=S3.Ap.concat("/pr/contact_emergency/create.iframe?person_id="+c);b&&(k+="&access="+b);d.get(k,function(h){var g=d('<div class="iframe-container">').hide().html(h);h=S3.Ap.concat("/pr/contact_emergency/create?person="+
c+"&controller="+m);b&&(h+="&access="+b);var l=g.find("form").attr("action",h);h=d('<a class="cancel action-lnk">'+f.cancelButtonText+"</a>").on("click",function(){l.slideUp("fast",function(){g.remove()});d(a).show()});l.find('input[type="submit"]').after(h);g.insertAfter(a).slideDown("fast",function(){e.siblings(".throbber").hide();g.find("#popup").show();l.show()})})},_editContact:function(a){var e=d(a),f=this.options,b=this;d(".iframe-container").remove();this._showAll();e.find(".pr-contact-actions").children().toggle();
var c=f.personID,m=f.controller,k=e.data("id"),h=f.access;a=S3.Ap.concat("/pr/contact/"+k+".iframe/update?person="+c);d.get(a,function(g){var l=d('<div class="iframe-container">').hide().html(g);g=S3.Ap.concat("/pr/contact/"+k+"/update?person="+c+"&controller="+m);h&&(g+="&access="+h);var n=l.find("form").attr("action",g);g=d('<a class="cancel action-lnk">'+f.cancelButtonText+"</a>").on("click",function(){n.slideUp("fast",function(){l.remove()});b._showAll()});n.find('input[type="submit"]').after(g);
e.after(l).hide();l.slideDown("fast",function(){l.find("#popup").show();n.show()})})},_editEmergencyContact:function(a){var e=d(a),f=this.options,b=this;d(".iframe-container").remove();this._showAll();e.find(".pr-contact-actions").children().toggle();var c=f.personID,m=f.controller,k=e.data("id"),h=f.access;a=S3.Ap.concat("/pr/contact_emergency/"+k+".iframe/update?person="+c);d.get(a,function(g){var l=d('<div class="iframe-container">').hide().html(g);g=S3.Ap.concat("/pr/contact_emergency/"+k+"/update?person="+
c+"&controller="+m);h&&(g+="&access="+h);var n=l.find("form").attr("action",g);g=d('<a class="cancel action-lnk">'+f.cancelButtonText+"</a>").on("click",function(){n.slideUp("fast",function(){l.remove()});b._showAll()});n.find('input[type="submit"]').after(g);e.after(l).hide();l.slideDown("fast",function(){n.show();l.find("#popup").show()})})},_deleteContact:function(a){if(confirm(i18n.delete_confirmation)){var e=a.data("id");d.post(S3.Ap.concat("/pr/contact/"+e+"/delete.json"));a.hide()}},_deleteEmergencyContact:function(a){if(confirm(i18n.delete_confirmation)){var e=
...
cssclass:"pr-contacts-editable",data:function(b){for(var c="{",m=1;10>m;m++)c+='"'+m+'":"'+m+'",';return c+('"selected":"'+b+'"}')},type:"select",submit:"ok"});return!0},_unbindEvents:function(){var a=d(this.element),e=this.eventNamespace;a.off(e);a.find(".pr-contact-add, .pr-emergency-add, .pr-contact-form").off(e);return!0}})})(jQuery);
