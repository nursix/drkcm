...
h=d("<video>").appendTo(f);h.css({width:"300",height:"300"});var q=f.dialog({title:"Scan QR Code",autoOpen:!1,modal:!0,classes:{"ui-dialog":"qrinput-dialog"},close:function(){e&&(e.stop(),e.destroy(),e=null)}});l.on("click",function(){p.hide();n.hide();h.show();q.dialog("open");e=new QrScanner(h.get(0),function(k){e.stop();h.hide();window.navigator.vibrate(100);let g="";try{g=a._parse(k)}catch(t){g=!1}!1===g?(g=k="",p.show()):n.show();a.options.keepOriginalInput||(k=g);b.val(g).trigger("change"+a.eventNamespace);
c.val(k).trigger("change"+a.eventNamespace);setTimeout(function(){q.dialog("close")},400)},function(){});e.start()})}else l.prop("disabled",!0)});this._bindEvents()},_clearInput:function(){this.hiddenInput.val("").trigger("change"+this.eventNamespace);d(this.element).val("").trigger("change"+this.eventNamespace)},_parse:function(b){var c=this.options,a=c.inputPattern;if(!b||!a)return b;a=(a=(new RegExp(a,"g")).exec(b))?(c=c.inputIndex)||0===c?"string"==typeof c?a.groups[c]:a[c]:b:!1;a===r&&(a=!1);
return a},_bindEvents:function(){let b=d(this.element),c=this.eventNamespace,a=this;d(".clear-btn",b.closest(".qrinput")).off(c).on("click"+c,function(){a._clearInput()});b.off(c).on("input",function(){a.hiddenInput.val(b.val().trim())});return!0},_unbindEvents:function(){let b=d(this.element),c=this.eventNamespace;b.off(c);d(".clear-btn",b.closest(".qrinput")).off(c);return!0}})})(jQuery);
