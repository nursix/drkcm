...
h.a+"</div></div>").hide().insertAfter(b(e+"_label").closest(".controls")).slideDown("fast");if(h.p){k._showPerson(h.p);h.b&&(k.imageURL.val(h.b),k.profilePicture=h.b,k._showProfilePicture());var f=k.flagInfo;h.f?f.val(JSON.stringify(h.f)):f.val("[]");f=k.permissionInfo;h.s!==n?f.val(JSON.stringify(h.s)):f.val("false");f=k.actionableInfo;var m=h.u;f.length&&(m!==n?f.val(JSON.stringify(h.u)):(m=!0,f.val("true")));h.d&&k._updateDetails(h.d,m);k.familyInfo.val(JSON.stringify(h.x||[]));k._showFamily();
...
"</button>").appendTo(c);d=b("<td>").append(e);e=b('<td class="member-info">').append(g);c=b("<td>").append(c);return b('<tr class="family-member">').data("member",a).append(d).append(e).append(c)},_updateFamilyStatus:function(a){let d=this.familyContainer,e=b(".family-member",d),c=this.eventType.val(),g=0,l=new Date,k=this._getEventType();if(k&&k.multiple)return!e.length&&this.familyInfo.val()&&this._showFamily(),b(".family-member",d).each(function(){var h=b(this),f=h.data("member").r,m=!a,q="";
if(f&&f.hasOwnProperty(c)){f=f[c];var r=f[1]?new Date(f[1]):null;if(null===r||r>l)m=!0,q=b('<div class="member-message">'+f[0]+"</div>")}h.find(".member-message").remove();m?(h.removeClass("member-selected"),b(".member-select",h).each(function(){b(this).prop("checked",!1).prop("disabled",!0)}),h.addClass("member-blocked"),m=b('<div class="member-message row">'),b('<div class="columns">').append(q).appendTo(m),h.find(".member-info.row").after(m)):(g++,h.removeClass("member-blocked"),b(".member-select",
h).each(function(){b(this).prop("disabled",!1).prop("checked",!0);h.addClass("member-selected")}))}),this._updateSelectAll(),this._updatePictureButtons(),g;this._hideFamily();this._updateSelectAll()},_updateSelectAll:function(){var a=0,d=0,e=!0;b(".member-select").each(function(){var g=b(this);g.prop("disabled")||(a++,g.prop("checked")?d++:e=!1)});var c=b(".member-select-all");a?c.prop("disabled",!1).prop("checked",e):c.prop("checked",!1).prop("disabled",!0);c=b(".submit-btn");b(".member-select").length?
(c.val(this.submitLabel+" ("+d+")"),d?c.prop("disabled",!1):c.prop("disabled",!0)):c.val(this.submitLabel)},_selectAll:function(a){var d=0;b(".member-select").each(function(){var e=b(this);e.prop("disabled")||(a?(e.prop("checked",!0).closest("tr.family-member").addClass("member-selected"),d++):e.prop("checked",!1).closest("tr.family-member").removeClass("member-selected"))});this._updateSelectAll()},_toggleSubmit:function(a){var d=b(this.element),e=[".check-btn",".submit-btn"],c=this.permissionInfo,
g=this.actionableInfo;if(a){var l=a=!1;c.length&&(c=c.val())&&(a=JSON.parse(c));a&&(l=!0,g.length&&(g=g.val())&&(l=JSON.parse(g)));a&&l&&(l=this._checkBlockedEvents());a&&l&&e.reverse()}c=d.find(e[0]);d.find(e[1]).prop("disabled",!0).hide().insertAfter(c);c.prop("disabled",!1).hide().removeClass("hide").show()},_clearAlert:function(){b(".alert-error, .alert-warning, .alert-info, .alert-success").fadeOut("fast");b(".error_wrapper").fadeOut("fast").remove()},_clearForm:function(a,d){b(".inline-throbber").remove();
a||this._clearAlert();d||b(this.idPrefix+"_label").val("");this._removePerson();this._removeFlagInfo();this._removeDetails();this._removeFamily();this._removeProfilePicture();this.blockingMessage&&this.blockingMessage.remove();this.imageURL.val("");b(".submit-btn").val(this.submitLabel);this._toggleSubmit(!1);a=b(this.idPrefix+"_label");a.trigger("focus").val(a.val())},_getEventType:function(){let a=this.eventType.val();if(!a)return null;let d=b("a.event-type-select",this.eventTypeSelect).filter(function(){return b(this).data("code")==
a});return d.length?{code:d.data("code")||null,name:d.data("name")||null,multiple:"T"==d.data("multiple")}:null},_setEventType:function(a,d){b('input[type="hidden"][name="event"]').val(a);b(".event-type-name",this.eventTypeHeader).text(d);this._updateEventType();this._updateFamilyStatus();b(this.idPrefix+"_person__row .controls").text()&&this._toggleSubmit(!0)},_clearEventType:function(){b('input[type="hidden"][name="event"]').val("");this._updateEventType();this._toggleSubmit(!1)},_updateEventType:function(){let a=
this.eventTypeHeader,d=b("a.event-type-select",this.eventTypeSelect).length,e=b('input[type="hidden"][name="event"]').val();var c=this.options;e?a.removeClass("challenge"):(c=0<d?c.selectEventLabel:c.noEventsLabel,b(".event-type-name",a).text(c));0==d?a.addClass("empty").addClass("disabled"):1==d&&e?a.removeClass("empty").addClass("disabled"):(a.removeClass("empty").removeClass("disabled"),e||a.addClass("challenge"))},_populateEventTypes:function(a){let d=this.eventTypeSelect.empty(),e=a.types;(a=
a.default)||1!=e.length||(a=e[0]);e.forEach(function(c){let g=c[0],l=c[1];c=c[2]?"T":"F";b('<a class="secondary button event-type-select">').text(l).appendTo(d).data({code:g,name:l,multiple:c})});a?this._setEventType(a[0],a[1]):this._clearEventType()},_selectOrg:function(a){let d=a.data("id");a=a.data("name");let e=this.orgHeader,c=this.orgSelect;this._clearForm();e.data("selected",d).addClass("selected");b("h4.org-name",e).text(a);let g=b('<div class="inline-throbber">'),l=this.eventTypeHeader.addClass("disabled"),
k=b(".event-type-name",l).hide().after(g),h=this;b.ajaxS3({url:this.options.ajaxURL+"?org="+d,type:"GET",dataType:"json",contentType:"application/json; charset=utf-8",success:function(f){h._populateEventTypes(f);g.remove();k.show();1<f.types.length&&l.removeClass("disabled")},error:function(){g.remove()}});c.slideUp("fast")},_selectEventType:function(a){let d=a.data("code");a=a.data("name");this._setEventType(d,a);this.eventTypeSelect.slideUp("fast")},_bindEvents:function(){var a=b(this.element),
d=this.idPrefix,e=this.eventNamespace,c=this;let g=this.orgHeader,l=this.orgSelect,k=this.eventTypeHeader,h=this.eventTypeSelect;g.on("click"+e,function(f){f.preventDefault();f.stopPropagation();if(g.hasClass("disabled"))return!1;h.slideUp("fast",function(){l.hasClass("hide")?l.hide().removeClass("hide").slideDown("fast"):l.slideToggle("fast")})});l.on("click"+e,"a.org-select",function(f){f.preventDefault();f.stopPropagation();c._selectOrg(b(this))});k.on("click"+e,function(f){f.preventDefault();
f.stopPropagation();if(k.hasClass("disabled"))return!1;l.slideUp("fast",function(){h.hasClass("hide")?h.hide().removeClass("hide").slideDown("fast"):h.slideToggle("fast")})});h.on("click"+e,"a.event-type-select",function(f){f.preventDefault();f.stopPropagation();c._selectEventType(b(this))});this.pictureContainer.on("click"+e,".toggle-picture",function(f){f.preventDefault();c._togglePicture()});a.find("a.cancel-action, .clear-btn").on("click"+e,function(f){f.preventDefault();c._clearForm()});b(".qrscan-btn",
a).on("click"+e,function(f){c._clearForm()});a.find(".check-btn").on("click"+e,function(f){f.preventDefault();c._checkID()});a.find(".submit-btn").off(e).on("click"+e,function(f){f.preventDefault();c._registerEvent()});d=b(d+"_label");d.on("input"+e,function(f){c._clearForm(!1,!0)});d.on("keyup"+e,function(f){switch(f.which){case 27:c._clearForm()}});a.on("change"+e,".member-select",function(){var f=b(this),m=f.closest("tr.family-member");f.is(":checked")?m.addClass("member-selected"):m.removeClass("member-selected");
c._updateSelectAll()});a.on("click"+e,".member-info",function(f){f.preventDefault();f=b(this).closest("tr.family-member").find(".member-select").first();f.prop("disabled")||f.prop("checked",!f.prop("checked")).change();return!1});a.on("change"+e,".member-select-all",function(){b(this).is(":checked")?c._selectAll(!0):c._selectAll(!1)});a.on("click"+e,"tr.family-all",function(f){if(!b(f.target).hasClass("member-select-all"))return f.preventDefault(),f=b(this).find(".member-select-all").first(),f.prop("disabled")||
f.prop("checked",!f.prop("checked")).change(),!1});a.on("click"+e,"button.member-show-picture",function(){var f=b(this).closest(".family-member").data("member");f.p?(c.profilePicture=f.p,c._showProfilePicture()):c._removeProfilePicture();c._updatePictureButtons()});return!0},_unbindEvents:function(){var a=b(this.element),d=this.eventNamespace,e=this.idPrefix;this.orgHeader.off(d);this.orgSelect.off(d);b("#event-type-toggle").off(d);b("#event-type-selector").find("a.event-type-selector").off(d);b(e+
"_label").off(d);a.find("a.cancel-action").off(d);a.find(".check-btn").off(d);a.find(".submit-btn").off(d);a.off(d);return!0}})})(jQuery);
