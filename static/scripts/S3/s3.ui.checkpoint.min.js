...
this.eventTypeHeader,c=b("a.event-type-select",this.eventTypeSelect).length,e=b('input[type="hidden"][name="event"]').val();var d=this.options;e?a.removeClass("challenge"):(d=0<c?d.selectEventLabel:d.noEventsLabel,b(".event-type-name",a).text(d));0==c?a.addClass("empty").addClass("disabled"):1==c?a.removeClass("empty").addClass("disabled"):(a.removeClass("empty").removeClass("disabled"),e||a.addClass("challenge"))},_populateEventTypes:function(a){let c=this.eventTypeSelect.empty(),e=a.types;(a=a.default)||
1!=e.length||(a=e[0]);e.forEach(function(d){let g=d[0],k=d[1];d=d[2]?"T":"F";b('<a class="secondary button event-type-select">').text(k).appendTo(c).data({code:g,name:k,multiple:d})});a?this._setEventType(a[0],a[1]):this._clearEventType()},_selectOrg:function(a){let c=a.data("id");a=a.data("name");let e=this.orgHeader,d=this.orgSelect;this._clearForm();e.data("selected",c).addClass("selected");b("h4.org-name",e).text(a);let g=b('<div class="inline-throbber">'),k=this.eventTypeHeader.addClass("disabled"),
