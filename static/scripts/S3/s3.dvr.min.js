...
a||this._clearAlert();c||b(d+"_label").val("");b(d+"_person__row .controls").hide().empty();this._removeProfilePicture();this._clearDetails();this._toggleSubmit(!1)},_bindEvents:function(){var a=b(this.element),c=this.idPrefix,d=this.eventNamespace,e=this,k=b(".zxing-button"),h=b("#event-type-toggle"),f=b("#event-type-selector");if(-1==navigator.userAgent.toLowerCase().indexOf("android"))k.addClass("disabled").on("click",function(g){g.preventDefault();g.stopPropagation();return!1});else k.on("click"+
d,function(){e._clearAlert()});h.on("click"+d,function(){e._clearAlert();f.hasClass("hide")?f.hide().removeClass("hide").slideDown():f.slideToggle()});f.find("a.event-type-selector").on("click"+d,function(){var g=b(this),l=g.data("code");g=g.data("name");b('input[type="hidden"][name="event"]').val(l);b(".event-type-name").text(g).removeClass("placeholder");b(c+"_person__row .controls").text()&&e._toggleSubmit(!0);k.each(function(){var p=b(this),q=p.data("tmp");q&&p.attr("href",q.replace("%7BEVENT%7D",
l))});f.slideUp()});a.on("click"+d,".toggle-picture",function(g){g.preventDefault();e._togglePicture()});a.find("a.cancel-action").on("click"+d,function(g){g.preventDefault();e._clearForm()});this.options.ajax&&(a.find(".check-btn").on("click"+d,function(g){g.preventDefault();e._checkID()}),a.find(".submit-btn").off(d).on("click"+d,function(g){g.preventDefault();e._registerEvent()}));a=b(c+"_label");a.on("input"+d,function(g){e._clearForm(!1,!0)});a.on("keyup"+d,function(g){switch(g.which){case 27:e._clearForm()}});
