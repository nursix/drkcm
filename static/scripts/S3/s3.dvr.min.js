...
a||this._clearAlert();c||b(d+"_label").val("");b(d+"_person__row .controls").hide().empty();b(d+"_flaginfo__row .controls").hide().empty();this._removeProfilePicture();this._clearDetails();this._toggleSubmit(!1)},_bindEvents:function(){var a=b(this.element),c=this.idPrefix,d=this.eventNamespace,e=this,k=b(".zxing-button"),h=b("#event-type-toggle"),f=b("#event-type-selector");if(-1==navigator.userAgent.toLowerCase().indexOf("android"))k.addClass("disabled").on("click",function(g){g.preventDefault();
g.stopPropagation();return!1});else k.on("click"+d,function(){e._clearAlert()});h.on("click"+d,function(){e._clearAlert();f.hasClass("hide")?f.hide().removeClass("hide").slideDown():f.slideToggle()});f.find("a.event-type-selector").on("click"+d,function(){var g=b(this),l=g.data("code");g=g.data("name");b('input[type="hidden"][name="event"]').val(l);b(".event-type-name").text(g).removeClass("placeholder");b(c+"_person__row .controls").text()&&e._toggleSubmit(!0);k.each(function(){var p=b(this),q=p.data("tmp");
q&&p.attr("href",q.replace("%7BEVENT%7D",l))});f.slideUp()});a.on("click"+d,".toggle-picture",function(g){g.preventDefault();e._togglePicture()});a.find("a.cancel-action, .clear-btn").on("click"+d,function(g){g.preventDefault();e._clearForm()});b(".qrscan-btn",a).on("click"+d,function(g){e._clearForm()});this.options.ajax&&(a.find(".check-btn").on("click"+d,function(g){g.preventDefault();e._checkID()}),a.find(".submit-btn").off(d).on("click"+d,function(g){g.preventDefault();e._registerEvent()}));
a=b(c+"_label");a.on("input"+d,function(g){e._clearForm(!1,!0)});a.on("keyup"+d,function(g){switch(g.which){case 27:e._clearForm()}});return!0},_unbindEvents:function(){var a=b(this.element),c=this.eventNamespace,d=this.idPrefix;b(".zxing-button").off(c).off("click");b("#event-type-toggle").off(c);b("#event-type-selector").find("a.event-type-selector").off(c);b(d+"_label").off(c);a.find("a.cancel-action").off(c);a.find(".check-btn").off(c);a.find(".submit-btn").off(c);a.off(c);return!0}})})(jQuery);
