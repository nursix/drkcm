...
"max-width":"100%"}),r=e("<video>").css({width:"640px",display:"block","margin-left":"auto","margin-right":"auto"}).appendTo(u),C=e('<button type="button">Capture Image</button>');C.addClass("primary button action-btn").css({display:"block",width:"100%"}).appendTo(u);var S=u.dialog({title:"Capture Image",width:640,autoOpen:!1,modal:!0,classes:{"ui-dialog":"capture-dialog"},close:function(){}});navigator.mediaDevices.getUserMedia({video:!0,audio:!1}).then(Y=>{r.show();S.dialog("open");let O=r.get(0);
O.srcObject=Y;O.play();r.add(C).off(c).on("click"+c,function(T){T.stopPropagation();T.preventDefault();g._captureImage(O);S.dialog("close").dialog("destroy");u.remove()})}).catch(Y=>{alert(`An error occurred: ${Y}`);u.dialog("destroy");g.captureButton.prop("disabled",!0).hide()})},_captureImage:function(g){let c=document.createElement("canvas");c.width=g.videoWidth;c.height=g.videoHeight;c.getContext("2d").drawImage(g,0,0,c.width,c.height);if(g=c.toDataURL("image/png"))this.fileName="capture_"+(new Date).valueOf()+
".png",this.extension="png",this._loadImage(g);c.remove()},_bindEvents:function(){let g=e(this.element),c=this.eventNamespace,u=this;g.on("change"+c,function(r){u._FileSelectHandler(r)});this.uploadTitle.on("click"+c,function(r){u.uploadContainer.slideDown("fast",function(){u.uploadTitle.html(i18n.upload_image)})});this.selectCrop.on("click"+c,function(){u._activateCrop()});this.crop.on("click"+c,function(){u._cropImage()});this.cancel.on("click"+c,function(){u._deactivateCrop()});this.uploadedImage.on("load"+
c,function(){u._initCropAPI()});this.dropArea.on("dragenter"+c,function(r){r.stopPropagation();r.preventDefault();u._FileHoverHandler(r)}).on("dragover"+c,function(r){r.stopPropagation();r.preventDefault()}).on("dragleave"+c,function(r){u._FileHoverHandler(r)}).on("drop"+c,function(r){r.stopPropagation();r.preventDefault();u._FileSelectHandler(r)});this.captureButton.on("click"+c,function(){u._openCameraDialog()});return this},_unbindEvents:function(){let g=e(this.element),c=this.eventNamespace;g.off(c);
this.uploadTitle.off(c);this.selectCrop.off(c);this.crop.off(c);this.cancel.off(c);this.uploadedImage.off(c);this.dropArea.off(c);this.captureButton.off(c);return this}})})(jQuery);$(function(){});
