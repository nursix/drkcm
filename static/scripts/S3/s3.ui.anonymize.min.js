...
e=c('<div class="inline-throbber">').css({display:"inline-block","margin-left":"1rem"}).insertAfter(d),f=function(){e.remove();d.show();k.prop("disabled",!1)},g=[];a.find(".anonymize-rule").each(function(){c(this).prop("checked")&&g.push(c(this).attr("name"))});if(!g.length)return b.resolve(!1);a=a.find('input[name="_formkey"]').first().val();a=JSON.stringify({apply:g,_formkey:a});c.ajaxS3({type:"POST",url:this.options.ajaxURL,data:a,dataType:"json",contentType:"application/json; charset=utf-8",success:function(){f();
