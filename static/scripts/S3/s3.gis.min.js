...
$.when(y(this.map_id)).then(function(){var n=f.strategies,h=n.length,m;for(m=0;m<h;m++){var w=n[m];if("OpenLayers.Strategy.AttributeCluster"==w.CLASS_NAME){w.deactivate();break}}for(f.events.on({loadend:N});f.map.popups.length;)f.map.removePopup(f.map.popups[0]);for(m=0;m<h;m++)if(w=n[m],"OpenLayers.Strategy.BBOX"==w.CLASS_NAME||"OpenLayers.Strategy.ZoomBBOX"==w.CLASS_NAME){w.bounds=null;w.triggerRead();break}void 0!=g.s3.layerRefreshed&&g.s3.layerRefreshed(f)},function(n){s3_debug(n)},function(n){s3_debug(n)}))}},
d=S3.gis.maps,e;for(e in d){var g=d[e];g.layers.forEach(c,{map_id:e,map:g})}};var pa=function(b,a){var c=i18n.gis_name_map?!0:!1;var d=new OpenLayers.TileManager;c=new OpenLayers.Map("center",{controls:[],displayProjection:q,projection:a.projection_current,fallThrough:c,maxResolution:a.maxResolution,maxExtent:a.maxExtent,numZoomLevels:a.numZoomLevels,theme:null,tileManager:d,units:a.units});S3.gis.maps[b]=c;c.s3={};c.s3.id=b;c.s3.options=a;c.s3.plugins=[];c.registerPlugin=function(e){e.map=this;this.s3.plugins.push(e)};
ra(c);sa(c);return c},qa=function(b){var a=b.s3,c=a.options,d=new GeoExt.MapPanel({xtype:"gx_mappanel",map:b,center:c.center,zoom:c.zoom,plugins:[]});a.mapPanel=d;var e={};e.map=d;a.portal=e;if(c.legend||c.layers_wms){e=b.layers;for(var g=d.layers.data.items,f=0;f<e.length;f++)e[f].legendURL&&(g[f].data.legendURL=e[f].legendURL),e[f].legendTitle&&(g[f].data.title=e[f].legendTitle),e[f].queryable&&(g[f].data.queryable=1)}e=ta(b);g=[e];c.wms_browser_url&&(f=ua(b))&&g.push(f);c.legend&&("float"==c.legend?
f=va(b):(f=new GeoExt.LegendPanel({title:i18n.gis_legend,defaults:{},autoScroll:!0,collapsible:!0,collapseMode:"mini"}),g.push(f)),d.legendPanel=f);d=a.plugins;f=0;for(var k=d.length;f<k;++f)d[f].setup(b),d[f].addToMapWindow(g);a.west_panel_items=g;c.window?aa(b):ba(b);var l=a.westPanelContainer;l.fireEvent("collapse");window.setTimeout(function(){l.fireEvent("expand")},300);e.root.eachChild(function(){this.eachChild(function(){if(this.isLeaf())this.on("checkchange",function(n,h){h||P(this.layer)});
else this.eachChild(function(){if(this.isLeaf())this.on("checkchange",function(n,h){h||P(this.layer)})})})});Ext.QuickTips.init()},ba=function(b){var a=b.s3,c=a.options,d=ca(b);b=da(b);c=new Ext.Panel({renderTo:c.renderTo,autoWidth:!0,titleCollapse:!0,height:c.map_height,layout:"border",items:[d,b]});a.mapWin=c};S3.gis.addMapPanel=ba;var aa=function(b){var a=b.s3,c=a.options,d=ca(b),e=da(b);d=new Ext.Window({cls:"gis-map-window",collapsible:!1,constrain:!0,closable:!c.windowNotClosable,closeAction:"hide",
autoScroll:!0,maximizable:c.maximizable,titleCollapse:!1,height:c.map_height,width:c.map_width,layout:"border",items:[d,e]});d.on("beforehide",function(g){g.maximized&&g.restore()});d.on("move",function(){b.events.clearMouseCache()});d.on("resize",function(g,f,k){842>k?$("#"+a.id).removeClass("a0 a1 a2 a3").addClass("a4"):1191>k?$("#"+a.id).removeClass("a0 a1 a2 a4").addClass("a3"):1684>k?$("#"+a.id).removeClass("a0 a1 a3 a4").addClass("a2"):2384>k?$("#"+a.id).removeClass("a0 a2 a3 a4").addClass("a1"):
